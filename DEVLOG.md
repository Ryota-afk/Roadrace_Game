...
6. **トレーニング室で選手が「ぼっ立ち」していてトレーニングしているように見えない問題**
   （ユーザー指摘・未修正）。原因は`domain/season/riderActivity.js`の`workSpotFor()`が
   「その部屋に椅子(kind==="chair")があれば座る、なければ什器の手前に立つ」という
   二択しか持たないこと。椅子はメディカル室・スカウト室にしか無いため、トレーニング室と
   メカニック室の選手は什器の脇にただ立つだけになる。ローラー台に跨がって漕ぐ、
   ダンベルを持ち上げる等の「作業ポーズ」が必要。ポーズを増やすと`PixelPerson`の
   スプライト追加が伴うため、ドット絵化ウェーブ（上記4番のA項①②）と一緒に検討すると
   効率が良い可能性がある。
...
8. マイライフの難易度調整。現状の課題（ユーザー指摘）：
   - 配合（血統ユース）を使わなくても能力値がカンストしてしまう
   - シーズン後半は「超晩成×成長力S×機材フルカンスト」の選手でレースに勝ち続ける
     以外の攻略ルートが実質無くなる（育成・戦略面の分岐が乏しい）
