...
> ゲーム内容の候補は移行の影響を受けずそのまま有効。**移行前の"次にやる予定"は C-2 なので、再開時の第一候補は引き続き C-2。**

### ゲーム内容
- **C-2 追加フォーマット／適性**（再開時の本命・次にやる予定）：チームTT・トラック等の新競技、石畳/クラシック専用の適性・特能
  （例「石畳巧者」＝モニュメントで能力+。ただしモニュメント判定を能力計算に通す必要あり＝`raceMeta.monument` を effAbilities まで届けるか、buildMyLifeSim で後掛けブースト）
