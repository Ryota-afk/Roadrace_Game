...
- **バランス調整パス**（2026-07に集中実施・すべて§4に記録）：シーズン順位実効化／地形別フィニッシュ決着
  ...
  - **配合まわりは点検済み（§4「配合点検＋二刀流を実効化」）**：成長力2.3倍差・配合限定特能とも健全に機能、
    弱点だったhybrid二刀流のみ実効化。爆発力→matingGrade/growthSteps/talentCapの変換も妥当。→ 概ね良好。
  - 残る調整候補＝危険度（インブリードの代償）・系統確立・特殊配合（唯一無二の名血）・生きた世界の各要素。
