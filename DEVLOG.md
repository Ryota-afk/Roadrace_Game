...
**残した検討事項（未着手）**：①レーダー2枚の間に区切り線が無く少し繋がって見える
②capが上がると外周が広がるため、成長しても図形が縮んで見える瞬間がある（数値と
ヘッダーの「外周=98」表記で担保しているが、外周を140固定にする案もある。序盤が
全体的に小さく見える代償あり）。
...
8. ✅ マイライフの難易度調整。2026-08に設計確定・Phase 1実装完了（上記「§17」参照。実測でユーザー指摘を
   ...
   **Phase 2（次回以降・メモのみ）**：イベントを能動発火（ボタン押下）から受動発火へ
   刷新し件数も拡充。新ステータス「運」（良いイベントの起きやすさ）を追加。
