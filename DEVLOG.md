...
- **バランス調整パス**（2026-07に集中実施・すべて§4に記録）：シーズン順位実効化／地形別フィニッシュ決着
  ...
  - **難易度カーブ診断で判明した副次課題（未着手・要判断）**：
    (1) **脚質が着順を大きく左右**する（決着地形に合った脚質でないと集団ゴールで沈む）。これは地形別決着の意図通りだが、
    プレイヤーへの導線（このレースは自分の脚質に向くか）をもっと明示してよい。
    (2) **hard/oniは"序盤ほど厳しい逆カーブ"傾向**：AI能力に上限があり自分は成長し続けるため、B1/A帯（自OVRが低い）が
    最も勝てず、PRO帯（育った頃）に楽になりがち。opt-in高難度としては許容範囲だが、B1/Aで全く表彰台に乗れないのは
    改善余地あり（例：低クラスのAI上限を少し下げる／grade帯を絞る）。数値ノイズが大きい領域なのでN大で慎重に。
  - 残る調整候補＝配合・爆発力・危険度・系統・特殊配合・生きた世界の各要素を回して数値を整える。
