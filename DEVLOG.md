...
- **Wave G-1（造形の土台改善・完了）**：方向は変えず、`Person.jsx`と`IsoRider.jsx`の両方に
  ...
  **検証**：既存Node全スイート（wavef3 1287・wavee3fixtures 229・wavef1 104・wavef2b 84・
  ...
  側で確認を依頼する。
- **Wave G-2（人物の8方向化・未着手）**：N/NE/E/SE/Sの5種を描画し反転で8方向をカバー。
  歩行サイクルの脚軌道も方向ごとに変える。`components/sprites/person/`への分割を検討
  （ファイル肥大化防止・CLAUDE.md §5）。
- **Wave G-3（自転車の3/4ビュー化・未着手）**：せん断投影（`iso: X=a*u, Y=-b*u+a*u*0.5`）で
  車体を3/4ビュー化。車輪は真円のまま（参考画像も同様の簡略化）。`IsoRider`に`view`引数
  （"side"=レース画面用に現行維持／"iso"=拠点画面用）を追加し、レース画面への影響を遮断する。
  3/4ビュー化はレース画面と共有する`IsoRider`の見た目を大きく変えるため、本弾では
  レース画面の回帰スクリーンショットを必須とする。
