...
11. **スプリント／ダンシングのモデルを描き直す**（`components/sprites/pixelBike.jsx`）。
   ...
   - **ユーザーが自分でモデルを修正する**ため、現行12コマ（BIKE/BIKE_B × normal/dancing/
     sprint × SE/NE）をPNGシートに書き出して受け渡し済み（scratchpadのrender_bike.py。
     同スクリプトを再実行すれば現行モデルをいつでも書き出せる）。
   - 書き出して並べた結果、**dancingがnormalとほぼ同じシルエット**（腰が上がっておらず
     座ったままに見える）で、**A/Bのクランク位相差も小さい**ことを確認済み。
   - 新しい参考画像を受け取ったら、CLAUDE.md §6（左右handedness整合性チェック）に従って
     抽出すること。**確定済みの別コマと同じ参考画像上でハンドル・前輪の位置を目視で
     突き合わせてから確定する**（過去にdancing_NEで長期化したバグの再発防止）。
