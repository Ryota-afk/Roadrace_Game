...
- **BIKE(A)のnormal_NEフレームが左右逆だったバグの修正（進行中・ユーザー確認待ち）**：
  「カーブ頂点で不自然」という報告を受け、当初は等角投影の性質上SE/NEの2方向しか
  持たないことによる角度近似誤差（構造的な限界）が原因と考え、参考画像追加などの
  対応案をユーザーに提示した。しかしユーザーから「角ではない、添付画像の6マス中
  右上（`BIKE(A).normal_NE`のflip=false）だけ左右が逆」と直接指摘があり、この
  頭部オフセット数値ヒューリスティックでは検知できていなかった（dancing/sprintの
  NEが逆符号でnormal_NEと一貫しないため、ヒューリスティック自体が信頼できないと
  判明）。ユーザー指摘を直接信頼し、`BIKE.normal_NE`を水平反転。ペダリング
  切り替え相手の`BIKE_B.normal_NE`も同じ向きだった（反転前は両者の頭部オフセットが
  一致していた）ため、A/B間の不整合（切り替えるたびに左右が入れ替わって見える）を
  防ぐ目的で同時に反転した（`fix_ne_mirror.mjs`、BIKE_B側への拡大適用はユーザー
  指摘の範囲を超える判断のため、確認待ちとして明示的に報告）。
  **検証**：修正前後の拡大レンダリングをユーザーへ送付し目視確認を依頼中。
  本番ビルド成功。
