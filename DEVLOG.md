...
## 19. 最終スプリント演出の姿勢が変わらない問題（2026-08・完了）

**ユーザー指摘**：「レース画面のラストスプリントで誰もダンシングやスプリント姿勢に
ならないのが気になる」。

**真因（実測で特定・2つある）**：
- **(1) 姿勢の判定がPRO帯基準の絶対値だった。** `RaceView.jsx`の`FinalSprintCinematic`は
  `surging = (c.kick||0) > 0.2` の一点で姿勢を決めていたが、`kick`は`(sprint - 72) / 24`
  というスプリント能力の絶対値換算なので、条件を満たすには**スプリント能力77超**が必要。
  実測すると該当者は**B1帯0% / A帯4.8% / PRO帯24.8%**。つまり下位クラスでは能力値経由で
  一度も発火せず、`finisher`/`kicker`特能持ち（能力に関係なく`kick=1`）の数名が一瞬なる
  だけだった。
- **(2) `dancing`は`<symbol>`すら生成していなかった。** `bikeCombos`が`normal`/`sprint`の
  2姿勢しか列挙しておらず、仮に判定が通っても`<use href="#bikesym-dancing-…">`の参照先が
  存在しない＝**選手が消える**。実際にはコード上`dancing`を渡す経路自体が無かったため、
  ダンシングは一度も画面に出たことがなかった。

**修正**：判定を「集団内の相対順位」へ変更し、姿勢一覧を単一の定数に集約した。
- `src/domain/shared/sprintPosture.js`（新設・純関数／JSX非依存＝Node単体テスト可能。
  CLAUDE.md §5に従い、肥大化している`RaceView.jsx`（1439行）へ直接書き足さず切り出した）
  - `pickDancerIds(contenders)` … `kick`降順で上位25%（1〜3名にクランプ）を
    ダンシング担当として選ぶ。**フィールド相対なのでどのクラスでも必ず数名が該当する。**
  - `sprintPosture(rem, isDancer)` … 最終直線は全員がもがく場面なので**既定を`sprint`
    （下ハンドル）**に変更。仕掛ける数名だけがゴール手前（`rem < 2.6`）で`dancing`、
    ゴール通過後（`rem <= 0.05`）は身体を起こして`normal`へ戻す。
  - `SPRINT_POSTURES = ["normal","sprint","dancing"]`
- `RaceView.jsx`側は`bikeCombos`を`SPRINT_POSTURES`から生成するよう変更。これにより
  **「姿勢を増やしたのに`<symbol>`を作り忘れて選手が消える」という(2)の事故が構造的に
  起きなくなる**（真因(2)の再発防止であり、単なる書き換えではない）。

**検証**：`npm run build`成功。Node単体テストで①境界値（`rem`の各しきい値）②`pickDancerIds`
の件数クランプ（1〜3名）③`sprintPosture`の戻り値が必ず`SPRINT_POSTURES`の要素であること
（＝未定義シンボル参照が原理的に起きないこと）を確認したうえで、**B1/A/PRO各クラス60レースの
シミュレーションで3姿勢すべてが60/60レースに出現**することを確認（修正前は同条件で
dancingが0/60、sprintがB1で0/60）。Playwrightの実機動作でも演出15フレームを走査し、
出現姿勢が`['dancing','normal','sprint']`の3種・**未定義シンボル参照0件**・エラー0件を確認。
なお演出の実画面スクリーンショットは、×4速度では選手が手前に来る時間が一瞬でフレームを
捉えられず取得を断念した（DOM上の`<use>`参照で3姿勢の描画自体は上記の通り実証済み）。

**追記（同日）：ゴールスプリートの尺を延長**。ユーザー指摘「ゴールスプリントの時間を
長くしてみて」を受け、`FinalSprintCinematic`の接近フェーズ`t1`（従来5.2〜6.2秒）・
退出フェーズ`t2`（従来1.9秒）を`CINE_STRETCH = 1.4`倍に延長（約7.1〜8.1秒→約9.9〜11.3秒）。
時間だけ延ばすと地面が流れる速度が遅くなり「もっさり」して見えるため（v39.16のスピード感
チューニングが無効化される）、接近距離`vtStart`と退出距離（`exitVt`の定数項）も同じ倍率で
延ばし、速度プロファイル（ease-outの体感速度）は変えずに尺だけ延ばした。これにより
上記のダンシング／スプリント姿勢が画面に映る実時間も比例して延び、姿勢の切り替わりを
視認しやすくなる。**検証**：`npm run build`成功。Playwright実機で演出を再生し、
出現姿勢`['dancing','normal','sprint']`の3種・エラー0件を確認（純粋な定数倍率変更のため
Node単体テストの対象なし）。

**追記（同日）：ゴール前のスローに選手の動きが連動していなかった問題を修正**。ユーザー
指摘「ゴール直前でスローになると思うんだけど、そこで選手の動きはスローになってない」。
真因：`PixelBikeUse`のペダリング切替（`Math.floor((t+phase)*9.0)%2`）に渡す`t`が実経過秒
`el`そのものだったため、カメラ・選手位置`vt`がイーズアウトで減速しても脚の回転は常に
実時間ペースで回り続けていた（車輪の回転は本来「進んだ距離」に比例するものなのに、
時間だけで駆動していたのが原因）。修正：`vt`の進み具合から作った仮想時間`pedalT =
(vt - vtStart) * PEDAL_K`をペダリング用の`t`として渡すよう変更。`PEDAL_K`は接近開始直後
（＝最も速い瞬間）のケイデンスがおおよそ従来の実時間ペースに一致するよう較正。これにより
スロー区間では脚の回転も自然に減速し、演出が完全に静止した後（`vt`が固定された後）は
脚も静止する（＝フリーズしたゴールシーンで脚だけ回り続ける不自然さも同時に解消）。
**検証**：`npm run build`成功。Playwright実機で①出現姿勢3種に変化なし②エラー0件③
先頭選手のクランク切替タイミングを実測し、接近序盤（0〜3秒）より終盤（6〜11秒、スロー
区間）の方が切替間隔が長くなる（＝ケイデンスが遅くなる）ことを確認。

## 20. 選手識別マーカーを「引き出し線＋名前タグ」方式へ統一（2026-08・完了）

**ユーザー指摘**：「星マーカーと水色の丸が選手の顔と被って非常にわかりづらい。そもそも
水色の丸はダメかも。星も改善の余地はあるかも」。手描きの案（人型アイコンから折れ線を
引き出し、離れた場所に名前を置く）を添付し、「マップ、スプリント問わず」統一を要望。
続けて「転生ライバル選手とかは赤で名前表示できたらいいかも」を追加指摘（確認の結果、
「殿堂選手＝isLegend＝引退後に衰えて後年に再登場する歴代選手」のこと）。

**設計確認（AskUserQuestionで合意）**：
- 最終スプリント演出でのタグ対象は「自分・エース・自チーム」のみ（全選手に付けると
  最大22名密集時にごちゃつくため）。
- 自分がエースを兼ねる場合は「水色地に黄色い縁取り」の1タグで両方を表現。
- 俯瞰マップ（上・側面）の名前ラベルも同じ引き出し線タグ方式に統一する。

**実装**：`RaceView.jsx`に共通ヘルパーを新設。
- `RiderNameTag`：選手の基準点から斜め上へ短い線を引き、先端に丸みのあるタグ（背景＋縁取り
  ＋名前）を置く純粋な描画コンポーネント。演出（幅340）とマップ（幅660）の縮尺差は`scale`
  引数で吸収。
- `riderTagKind(r)`：演出用の対象判定（自分＝self／自分兼エース＝selfAce／エース＝ace／
  自チーム＝mate／対象外＝null）。全選手ではなくこの4種のみタグを出す。
- `mapTagKind(r)`：マップ用（上記に加えてrival＝ライバル、legend＝殿堂選手、other＝
  それ以外の先頭3名も対象。従来のlabelIds選出ロジックは変更していない）。
- `RIDER_TAG_STYLE`／`riderTagIcon`：色とアイコンの対応表。自分＝水色🚴／エース＝黄色★／
  自チーム＝青／ライバル＝赤🔥／殿堂選手＝赤系(別トーン)🏛／その他＝白。**自分＝水色・
  エース＝黄色という配色は俯瞰マップのマーカーリング／凡例と同じものを踏襲**（従来、
  マップの名前ラベルだけ自分を黄色で表示しており、マーカーリングの水色と食い違って
  いた実装ミスも今回のタグ化と合わせて解消）。
- 最終スプリント演出：従来の★（頭上19px固定）と水色の丸（顔に直接重なる半径10の輪）を
  廃止し、`RiderNameTag`に置き換え。選手ごとに`riderHash01`で引き出し位置を微妙にずらし、
  複数タグが完全に重ならないようにした。スプライト描画とは別パスで最後に重ね描きし、
  隣の選手のスプライトにタグが隠れないようにした。
- 俯瞰マップ（上・側面）：既存の直上テキストラベルを同じ`RiderNameTag`に置き換え。
  側面マップは従来ラベル自体が無かったため新規追加。
- 殿堂選手（isLegend）対応：`ridersUi`・最終スプリント演出の`contenders`スナップショット
  双方に`isLegend`フラグを追加し、マップの`labelIds`選出条件にも追加（演出側は自分／
  エース／自チームのみという合意スコープを維持し、対象に含めていない）。

**検証**：`npm run build`成功。Playwright実機でマイライフレースを実行し、①俯瞰マップで
複数のタグ（ライバル＝赤🔥、先頭独走選手＝白）が引き出し線付きで表示されること②最終
スプリント演出でエースのタグ（★・引き出し線）が選手の顔に重ならず表示されること③
コンソール／ページエラー0件、をスクリーンショットで確認。殿堂選手（isLegend）表示は
本セッションで実際に発生する場面には遭遇しなかったため、色分けロジックの静的コード
確認までに留まる（他の`mapTagKind`分岐と同じ仕組みのため個別の追加リスクは低いと判断）。

## 21. イベントによる能力変化を必ず明示（2026-08・完了）

**ユーザー指摘**：「イベントで起きた能力変化などは必ず明示したほうがいいです」。

**調査で判明した具体的な漏れ（3箇所）**：
- **弟子の指導イベント**（`mlResolveProtegeEvent`）：結果文が`ch.result`（フレーバーのみ）
  で、実際に動く弟子の絆・鍛錬・地力、および師（プレイヤー自身）の能力・疲労・監督評価が
  一切表示されていなかった。
- **オフシーズンの過ごし方**（`ML_OFFSEASON_CHOICES`）：「海外武者修行」は全能力+4・疲労+20、
  「国内自主トレ」は全能力+2という地味でない増減があるにもかかわらず、結果文には数値が
  一つも出ていなかった。
- **人生の岐路イベント**（`ML_CROSSROADS`）：既に一部の選択（怪我からの復帰系）には
  `resultNote`という数値明示の仕組みがあったが、**特殊能力の獲得だけを説明し、同じ選択で
  同時に起きている能力値-1〜-5の増減は素通りしていた**。「教えを胸に、独り立ちする」
  （恩師との別れ）は全能力+3するのに`resultNote`自体が無く完全に無言だった。

**修正方針**：個々のイベント文言に数値を手書きで足すのではなく、**before/afterの能力値を
機械的に比較して差分だけを拾う**共通ヘルパーを新設し、記述漏れが構造的に起きないようにした。
- `logic/support.js`に`abilityDeltaSummary(player, prevPlayer)`を新設（`AB_KEYS`を全走査し、
  変化があったものだけ「フラット+3・登坂-3」のように文字列化）。
- `controllers/mylife/career.js`の`mlResolveCrossroads`・`mlResolveOffseason`はこれを結果文に
  必ず併記するよう変更（`ML_OFFSEASON_CHOICES`に持たせていた手書きの数値summaryは、
  `addAb()`が成長キャップで頭打ちする場合に実際の増減とズレるため削除し、機械比較に統一）。
- `controllers/mylife/event.js`に`protegeEffectSummary(pd, md)`を新設し、
  `mlResolveProtegeEvent`の結果文に併記（弟子の絆・鍛錬・地力／師の能力・疲労・監督評価）。

**検証**：Node単体テストで①`abilityDeltaSummary`が変化した項目だけを正しく文字列化する
こと・変化なしで空文字になること②`ML_CROSSROADS.injury`の3択すべてで空でない差分文字列が
出ること（従来は一部無言だった）③`mentor_graduation`の+3も差分に出ること④
`ML_OFFSEASON_CHOICES`の3択すべてで能力or疲労の変化が可視化されること⑤
`protegeEffectSummary`が弟子・師双方の項目を正しく整形することを確認（12件全通過）。
`npm run build`成功。Playwrightでマイライフ起動までのコンソール／ページエラー0件を確認
（岐路・弟子・オフシーズンの各画面は発生条件が確率的で本セッション内では実地到達できな
かったため、この3画面のスクリーンショット確認は次回機会に持ち越し）。

## 22. UIテキストの作り込み 第1波（2026-08・完了）

**位置づけ**：次のアクション#9（★最優先・恒常タスク／CLAUDE.md §7）の初回着手分。
「あらゆる画面」が対象のため一度に全部はやらず、**3波に分ける方針でユーザー合意**。
本項は第1波（当初指摘された2点＋同じ型で確実に直せるもの）。

**着手前に全画面を棚卸しして判明した事実**：ユーザーが最初に指摘した「（生涯不変）」
「（外周=98）」は**CLAUDE.md §7に例として書かれただけで、実コードには手つかずで残っていた**。
さらに**マイライフ（`screens/mylife/hub.jsx`）とシーズン（`screens/season/hub/riders/list.jsx`）に
同一マークアップが重複**しており、文言を直すには2箇所を触る必要がある状態だった。

**実施した内容**：
- **レーダー2枚を共有部品化**：`components/RadarChart.jsx`に`AbilitySoshitsuRadarPair`を
  新設し、上記2画面の重複マークアップを置換（CLAUDE.md §5）。以後この種の文言修正は
  1箇所で済む。
- **`（生涯不変）`を削除**（§7(a)：あると親切かもしれない程度の補足は、画面をごちゃつかせる
  分だけマイナス）。
- **`（外周=88）`を見出しから撤去し、上限値をレーダー図の右下隅へ移設**。`RadarChart`に
  `corner={{label,value}}`を追加し「上限 / 88」の2行で控えめに置いた。**説明文を足す方向では
  なく図そのもので伝える**（§7(c)）。あわせて「外周」という実装側の語彙も画面から消えた
  （§7(b)）。※置き場所はユーザー確認のうえ「左（能力）グラフの右下隅」に決定
  （上限は能力グラフ側の値のため）。
- **`💪 練習（focus中心）`のラベルが変数名`focus`を生で出していた**のを、実際に選択中の
  能力名（`AB_LABEL[r.focus]`）を出すよう修正。すぐ上の練習メニューでは「スプリント強化」等と
  日本語で選ばせているのに、ボタンだけ英語の内部名という不整合だった（§7(b)）。
- **`才能キャップ+5` → `才能で上限+5`**（「キャップ」は§7(b)でユーザーが名指しした実装語彙）。
- **ユーザーに見える生の略語`CP`を全て「クリアポイント」へ統一**（`career.jsx`／`meta.jsx`／
  `season/intro.jsx`／`create.jsx`／`help.jsx`）。`CPショップ`は`クリアポイント交換所`に改称。
  同じ画面群の中でも「累計クリアポイント：0pt」と「CP残高」が混在していた表記ゆれも解消。
  コード内コメントの`CP`は変更していない（ユーザーに見えないため）。
- **バージョン番号入りの開発用見出しを2つとも撤去**：`MODE SELECT — v14`（`meta.jsx`）と
  **`SEASON MODE v12`（`season/intro.jsx`）**。後者は棚卸しで発見した同型の不具合で、
  §7の「1画面直して終わりにせず横断的に洗い直す」に従い同時に対応した。なお#10の残り
  （スタート画面の自チーム情報の非表示・説明文の誤り修正）は本波のスコープ外。

**検証**：`npm run build`成功。Playwrightで**マイライフ・シーズン両方の選手カードを実際に開き**、
①旧文言（`外周`／`生涯不変`／`focus`／`才能キャップ`／生の`CP ×`／`SEASON MODE v12`）が
**すべて画面から消えていること**②新しい`上限`表示が出ていること③練習ボタンが
`💪 練習（ｽﾌﾟﾘﾝﾄ中心）`と実際の能力名になっていること④レーダーが両画面で正しく描画される
こと（svg polygon 10個）⑤コンソール／ページエラー0件、をテキスト検査とスクリーンショットで確認。

**第2波以降に残した項目（棚卸しで発見済み・未着手）**：
- (c)小さい文字での仕組み解説群：`疲労（90超で故障リスク・60未満なら急いで回復させる必要は
  ありません）`／`能力88以上＝限界突破（バーの薄い帯＝上限までの伸びしろ…）`／
  `コース適性 S〜G（種目別の総合地力／★＝…）`／シーズン選手一覧冒頭の
  `練習指定能力の伸びはトレードオフ（×0.9）で…`（「トレードオフ（×0.9）」も実装語彙）など。
  これらはバーや図の見せ方自体を変えて注釈を減らす方向で検討する。
- (a)`📻 レース作戦（結果に反映されます）`の「結果に反映されます」等、実装者の不安から
  来る保証文。
- 未棚卸しの画面（シーズンの施設・市場・記録カテゴリ、マイライフの配合まわり）の洗い直し。

## 23. UIテキストの作り込み 第2波（2026-08・完了）

**位置づけ**：次のアクション#9の第2波。ユーザーから「**抜け漏れないようにね**」との指示が
あったため、**着手前に全画面（screens/ と components/ の全.jsx・53ファイル）を機械抽出で
棚卸し**してから作業した（`fontSize`と日本語文字列を突き合わせ、候補403件を目視で分類）。
レース実況・引退の語り・ライバル対話などの**フレーバー文は「作品の中身」なので対象外**とし、
直したのは仕組みの解説文・開発語彙・重複した保証文のみ。

**A. 英語の開発用見出しを撤去**（第1波の`MODE SELECT — v14`／`SEASON MODE v12`と同型）
- `MY LIFE — キャラクター作成`（create.jsx）→ `キャラクター作成`
- `SCOUT POLICY — 初年度（4月）のスカウト方針`（intro.jsx）→ `初年度のスカウト方針`
- `SPONSOR — 今季のメインスポンサーを選択`（intro.jsx）→ `今季のメインスポンサー`
- **棚卸しで新たに発見した常時表示の2件**：マイライフ全画面のヘッダー`MY LIFE — クラスB1…`
  → `マイライフ — …`、シーズン全画面のヘッダー`SPONSOR: カワセミ電工…` → `スポンサー …`

**B. 残る開発語彙を平易な日本語へ**
- **`成長力（growthPow A以上）`（youth.jsx）＝変数名の生リーク**を`成長力A以上`へ。
  第1波の`練習（focus中心）`と同じ型の見落としだった。
- `トレードオフ（×0.9）で指定外に一部融通されます` → `1割ぶんが他の能力へ回ります`
- `金特`（パワプロ由来の略語）を全廃し`金の特殊能力`へ統一（help/create/youth/panels）
- `連携（ケミストリー）` → `チームの連携`（同義語の重複併記を解消）

**C. 注釈をやめて図で伝える（本波の中心）**
- **`疲労（90超で故障リスク・60未満なら急いで回復させる必要はありません）`**：
  `FatigueBar`は元々**60/90で色が変わり90に赤い目盛りがある**＝しきい値を図で表現できて
  いたのに、呼び出し側が同じことを文章で二重に説明していた。**注釈を消し、代わりに90以上を
  薄い赤の危険域として塗った**（目盛りの右＝危ない領域、と絵で分かる）。マイライフ・
  シーズン両方の呼び出し側から注釈を削除。
- **`能力{cap}以上＝限界突破（バーの薄い帯＝上限までの伸びしろ・数字の小さな+も伸びしろ）`**：
  `AbilityGrid`が既に薄い帯・上限目盛り・金色・`+{room}`で全て表現していたので**全削除**。
  図に出ていない`才能で上限+X`だけを残した。
- `コース適性 S〜G（種目別の総合地力／★＝今月のレースが有利とする種目）`（9.5px）→
  `コース適性`のみ。文字グレードも★も図で見えている（シーズン側が説明なしの`種目別適性`
  だけで成立していることが根拠）。
- `フォーム（好不調）`→`フォーム`、`活力（伸びしろの芯）`→`活力`。隣に「平常／好調／ピーク」
  「充実／やや消耗／枯渇気味」が出るので言い換えは不要（「芯」は比喩で伝わらない）。

**D. 保証文・自明な補足を削除**
- `📻 レース作戦（結果に反映されます）` → `📻 レース作戦`（実装者の不安から来る保証文）
- `🎬 今月のアクション（1つ選ぶと1ヶ月進みます）` → `— 1つ選ぶと翌月へ`
- `🎯 専門トレーニング（狙いを絞って強化・1ヶ月消費）` → `🎯 専門トレーニング`
- `📂 メニュー（開くだけ・月は進みません）` → `（月は進みません）`

**E. シーズン各画面の長い解説文を圧縮**（OBコーチ／新人スカウト／FA／トレード／チーム状況／
選手一覧ヘッダー／レース出走人数／当日変更／チーム名／レジェンド招聘／解禁コンテンツ）。
いずれも2文以上→1文、または削除。

**F. 凡例の整理**
- **レース画面の凡例が10px・10項目に膨れていた**。うち「牽引中／次に牽引予定／エース発射」は
  **マップ上に役割バッジとしてテキストで既に描かれている**ため凡例側は重複。集団の振る舞いを
  説明する一文も削除し、**絵だけでは決して分からない「どの印が誰か」だけ5項目に絞った**。
- `panels.jsx`の通算タイトル・特能図鑑の説明、マイライフの選手成績パネルの説明も圧縮。

**検証**：`npm run build`成功。**旧文言19語をブラックリスト化したPlaywrightスクリプト**で
両モードを実際に操作し（モード選択→マイライフ作成→本編→詳細折りたたみ→メニュー、
シーズンintro→新規設定→本編→選手一覧／チーム状況／OBコーチ／スカウト／引き抜き／
カレンダー）、**各画面のinnerTextを走査して残存0件・コンソール／ページエラー0件**を確認。
主要6画面のスクリーンショットで表示崩れがないことも目視確認した。

**作業中に自分で作り込んだ不具合と、その検出**：一括置換の際に(1)キリル文字`создチーム`の
混入(2)`meta.jsx`で`<b>1つの世界</b>`が文から切り離され孤立、という2件の破損を作った。
いずれも**置換直後の`grep`と再ビルド・実機確認で検出して修正済み**。以後、文字列一括置換の
あとは非日本語文字（`\p{Cyrillic}`等）の混入スキャンをかけることを習慣にする。

**【反省】棚卸しスクリプトの穴と、そこから得た教訓**：本波の完了報告直後にユーザーから
「本当にテキスト面終わりかな？」と問われて再監査したところ、**取りこぼしが3件**見つかった
（`dynasty.jsx`の系譜ツリー・因子図鑑の解説文、`career.jsx`の監督転身の説明）。
原因は棚卸しスクリプトが`>テキスト<`という**単行のJSXテキストノードしか拾えない正規表現**
だったこと。複数行に分けて書かれた文（`<div ...>` の次の行から本文が始まる形）は
構造上ヒットしなかった。`dynasty.jsx`自体は走査対象に入っていたのに、該当行だけが
すり抜けていた＝**「ファイルを見た」と「中身を見切った」は別物**という教訓。
**次回以降のUI棚卸しでは、単行正規表現に頼らず、(a)タグを剥がして日本語文字列の連続を
行またぎで結合する方式と、(b)実機レンダリング結果のテキストノード走査、の2系統で
突き合わせること**（後者は密度計測にもそのまま使える）。

**第3波の3候補は「削らなくてよい」とユーザー判断済み（現状維持で決着）**：
- ヘルプ画面（`mylife/help.jsx`・`season/hub/help.jsx`）の本文。
- `OVR`という略語。
- シーズンヘッダーのスポンサー行の情報量。
（いずれも上記「第3波（未着手）」として提示したが、ユーザーから「1.2.3ともに削らないで
良い」と回答。以後この3点はUIテキスト作り込みの対象から外す。次のアクション#9からも
除外し、代わりに下記#15「導線・ユーザビリティ」を新設。）
...
疲労75未満→出走／それ以外→練習、の4段。ラベル文言・onClickは呼び出し側
（`hub.jsx`）が組み立てる（UI都合の文言を混ぜないためkeyとreasonだけを返す設計）。
Node単体テスト19/19件パス（境界値・優先順位・reason非空を検証）。

**`components/ui.jsx`の`Btn`に`role`プロップを追加**：色をボタンごとの場当たり指定
から「意味」の指定へ変更。`primary`=黄塗り（今の画面のおすすめ、1画面1つ想定）／
`month`=黄枠線（月が進む他の行動）／`menu`=グレー枠線（月が進まない・開くだけ）／
`danger`=赤枠線（取り返しがつかない操作のみ。前向きな決断でも「元に戻せない」ことを
示すため引退・データ消去にだけ使い、単に月が進むだけの操作には使わない）。
`role`未指定なら従来の`color`/`outline`をそのまま使うため既存呼び出しは無改修で動作。

**`screens/mylife/hub.jsx`の`mylife_main`を全面再構成**：
- 選手カード直下に新設「🎬 今月の行動」ブロックを追加。`mlNextAction()`の結果を
  黄塗りの強調ボタン＋理由1行で表示し、その下に非推奨の他の選択肢（レース/休養/練習の
  うち推奨以外）を黄枠線で並列表示、専門トレーニングは折りたたみへ格納。
- 判断材料となる「今月のレース」（戦術選択・天候予報を統合）と「今月の練習メニュー」を
  行動ブロック直後へ移動（旧位置は画面下部だった）。
- 監督指示／先月の成長／新聞／弟子／状況（📋）はその後段へ。
- 📂メニュー8ボタンを個別の色指定からすべて`role="menu"`（グレー枠線）へ統一。
- ⚙その他・キャリア管理を**閉じた状態のデフォルト折りたたみ**に変更。弟子育成の勧誘は
  前向きな意思決定なので`role="menu"`のまま、ラストレース引退／静かに引退／
  最初からやり直すのみ`role="danger"`（赤）にした。

**検証**：`npm run build`成功。Playwrightで新規デビュー→本編表示まで進め、
①今月の行動ブロックが黄塗りボタン＋理由で正しく表示され、推奨と同じ選択肢が
下の並列ボタンから正しく除外されていること、②メニュー8ボタンが均一グレーになった
こと、③その他折りたたみを開くと赤枠の引退系ボタンが正しく分離されていること、を
スクリーンショットで確認。コンソール／ページエラー0件。
**密度（末端テキスト要素数・ページ高さ）before/after**：109個/1830px → 96個/1644px。
（10px以下の内訳のうちレーダー軸ラベルなど図の一部は元々問題視していないため、
この減少は主に画面上部の情報整理によるもの。）

**副産物として見つかった第3の取りこぼし（`金特`のdata/controllers/domain残存）**：
本Wave完了直後の恒例チェックとして、旧文言ブラックリストのPlaywright回帰スクリプト
（`wave2_verify.mjs`）を通したところ、第2波（§23-B）で「全廃」したはずの`金特`が
シーズンの**スカウト**・**引き抜き**画面に再出現していることが判明した。原因は
第2波の棚卸し対象が`screens/`と`components/`のみで、**`data/`・`controllers/`・
`domain/`・`state/`配下のユーザー可視文字列（能力の説明文・ショップ項目名・
キャリアログ／通知メッセージ）が最初から棚卸しの範囲外だった**こと。第2波の「反省」
（単行正規表現の穴）とは別種の穴＝**そもそも走査対象ディレクトリが足りていなかった**
という、Wave2完了報告後にこれで3件目の取りこぼし発覚となった。
修正した6ファイル・10箇所（文脈に応じ`金の特殊能力`/`金`/`金の状態`と訳し分け）：
`controllers/mylife/month.js`・`controllers/season/month.js`（覚醒ログ）、
`controllers/season/roster.js`（ユース加入ログの継承表記）、`data/abilities.js`
（`pave_sp`等3能力のdesc）、`state/state.js`（CPショップ`m_gold`のlabel/desc）、
`domain/mylife/createChar.js`（天啓・英才の証・血統継承ログ計4箇所）。
コメント内の`金特`（実装者向け・非表示）は方針通り対象外のまま残した。
**検証**：`npm run build`成功。`wave2_verify.mjs`を再実行し「なし（全て消えている）」
「errors: 0件」を確認。`wave15_verify.mjs`（本Waveの回帰スクリプト）も再実行し
上記の今月の行動ブロック等が引き続き正しく描画されコンソールエラー0件であることを
再確認した。

## 25. 素質ランク・伸びしろヒントの非公開時ランク圧縮＋才能の片鱗の成長力リーク修正（2026-08・完了）

**位置づけ**：ユーザーから「マイライフの素質ランクの判定、成長力を隠したせいで変になってない？
才能の片鱗で成長力がわかっちゃったりする」との指摘。§18（成長力のマスク化）で
`mlTalentRank`／`potentialHint`に`revealPow`引数を足し、非公開期間中は成長力由来の加点を
ゼロ化する対応を入れていたが、副作用の実測・デビュー特典文の点検までは行っていなかった。

**問題1：才能の片鱗が成長力の等級を生でリーク**：デビュー特典「🌱 才能の片鱗」のノート文が
`秘めた伸びしろを感じさせる（成長力${before}→${player.growthPow}）`と、変化前後の等級
（例：C→B）を直書きしていた。同じ画面の他の表示は🔒???でマスクしているのに、この一言だけ
生の値が読めてしまう明確な情報漏洩バグだった。`domain/mylife/createChar.js`のノート文から
before/afterの具体的な等級表記を削除し「秘めた伸びしろを感じさせる（成長力が上がった。
詳細は3年目に判明する）」に変更。Playwrightで60回リロールし実際に5回発生させ、
等級表記（`[SABC]→[SABC]`パターン）が一切出ないことを確認した。

**問題2：非公開時、素質ランクが実質常にCへ潰れる**：`mlTalentRank`は成長力の寄与
（`powScore*2`、最大+6）が他の全要素の合計より大きい式になっており、非公開時に
`powScore`をゼロ化すると公開時と同じしきい値（2.5/5.8/8.5/11.5）では上位ランクに
届かなくなる。実際のデビュー候補（3経歴×5脚質、8万体）でシミュレーションした結果：

```
公開時（成長力込みで採点した場合）：SS 0.0% / S  0.4% / A 11.0% / B 44.8% / C 45.2%
非公開時（現行のまま。デビュー画面は常にこちら）：SS 0.0% / S 0.0% / A  0.0% / B  3.9% / C 96.1%
```

デビュー画面（`mylife_scout`）は`mlGrowthPowRevealed(ml)`が常に`false`を返す1年目にしか
出現しないため、上記「非公開時」が実質的にプレイヤーが常に目にする分布そのものだった。
`potentialHint`（マイライフ本編の「伸びしろ」表示）も同型の問題を抱えており、非公開中は
理論上「伸びしろ大」に到達不可能（視認可能スコアの最大値が3、しきい値が5）で常に中/小
だけが出ていた。

**修正**：両関数とも、`revealPow=false`のときだけ使う専用しきい値を新設した
（`revealPow=true`側の式・しきい値は無改修＝3年目以降・シーズンモードの挙動に影響なし）。
非公開時の視認可能スコア（成長力ゼロ化後のスコア）の分布を同じ8万体シミュレーションで
実測し、公開時の各ランク比率に近づくよう較正：
- `mlTalentRank`：非公開時のしきい値を`{B:0.5, A:2.1, S:3.8, SS:4.8}`に変更
  （`logic/support.js`）。
- `potentialHint`：非公開時のしきい値を`{中:2, 大:3}`に変更（視認可能スコアが1〜3の
  離散値しか取らないため、公開時と同じ較正手法で自然な等分割となる整数値を採用）。

**検証**：Node単体テストで新旧の2点を確認 — ①非公開時、`growthPow`をS/Cに変えても
スコア・ランクが完全一致すること（成長力リークが再発していないことの再確認、500件）、
②較正後の分布（4万体シミュレーション）が単一ランクへの collapse から脱していること
（`mlTalentRank`非公開：B59%/A15%/C25%/S0.7%、`potentialHint`非公開：中34%/大34%/小33%）。
`npm run build`成功。Playwrightでマイライフのデビュー候補を25回リロールし、実際に
A/B/SSと複数ランクが出ること、成長力は引き続き🔒???表示のままであることを画面上でも確認した。

## 26. 能力表示の重複統合＋用語統一（コース適性/種目別適性）（2026-08・完了）

**位置づけ**：ユーザーから「シーズン・マイライフ両方で、能力表示をレーダーだけにしてもいい気がする」
「同じ要素を2回説明しているもの・同じものを違う表現で説明しているものがあれば統合・統一したい」
との指摘。着手前にExploreエージェントで全画面の能力表示コンポーネントを棚卸しし、実際に
重複している箇所を特定してから、CLAUDE.md §1に従いユーザーに適用範囲を確認して実装した。

**確認できた重複**：マイライフ本編の自分の選手カードとシーズンの選手一覧カードの2箇所で、
`AbilityGrid`（能力5項目の棒グラフ）と`AbilitySoshitsuRadarPair`（同じ5項目のレーダー）が
同一データを二重表示していた。素質6項目（加速力/体格/メンタル/突破力/安定感/運）も
`SubStatLine`（テキスト）とレーダーで同様に二重だった。この2箇所以外（デビュー画面の
素質診断・FA市場・トレード・引き抜き市場・レース当日の出走選手選択/作戦変更カード）は
`AbilityGrid`のみでレーダーとの重複が無いため対象外（ユーザー確認済み・密なリスト画面に
レーダーは重すぎるとの判断）。

**用語ゆらぎ**：`DisciplineGrid`（コース種別ごとのS〜G適性評価）は完全に同一コンポーネント・
同一データ（`disciplineScore`）なのに、マイライフでは「コース適性」、シーズンでは
「種目別適性」と表記が割れていた。「コース適性」（マイライフ側・より短い既存表記）に統一。

**実装**（`screens/mylife/hub.jsx`・`screens/season/hub/riders/list.jsx`）：
- 両画面とも`AbilityGrid`・`SubStatLine`の呼び出しを削除。
- `DisciplineGrid`（見出しを「コース適性」に統一）を上、`AbilitySoshitsuRadarPair`
  （能力レーダー＋素質レーダー）を下に、**常時表示**で配置（マイライフは元々レーダーが
  常時表示・棒グラフ側が折りたたみ内だったため折りたたみごと廃止。シーズンは逆に棒グラフが
  常時表示・レーダー側が「▼ レーダーで見る」トグルの中だったため、トグルボタンと
  対応state（`radarRiderId`/`setRadarRiderId`、`hooks/useSeasonGame.js`）ごと削除）。
- マイライフ側の「才能で上限+{X}」注記（配合の才能キャップがある選手のみ表示、棒グラフでは
  表現できない情報としてWave2で意図的に残した経緯があるもの）はレーダー直下に移設して維持。

**検証**：`npm run build`成功。Playwrightでマイライフ（デビュー→本編）・シーズン
（新規設定→選手一覧）の両方を実際に開き、①「種目別適性」の文言が画面上に一切残っていない
こと、②「コース適性」が両モードで表示されていること、③削除したはずの折りたたみ文言
（「数値・伸びしろ…を見る」「レーダーで見る」）が残っていないこと、をinnerText走査で確認
（3項目×2モード＝6件全てOK）。コンソール／ページエラー0件。スクリーンショットで
レイアウト崩れがないことも目視確認した。

**次回以降の宿題（ユーザーメモ・今回スコープ外）**：能力上限の収束問題（後述の
次のアクション#16）はバランス変更を伴う大仕事のため、今回のUI整理とは切り離して
別途設計を詰める。
...
9. **ゲーム内の無駄なテキストを削り、「ゲームとして良いUI」に作り込む。**
   **【進捗】第1波（§22）・第2波（§23）を実施済み。第2波では全53画面ファイルを機械抽出で
   棚卸しし、フレーバー文を除く仕組み解説・開発語彙・保証文を一掃した。第3波の3候補
   （ヘルプ画面の本文／`OVR`／スポンサー行）は「削らなくてよい」とユーザー判断済みで決着
   （現状維持）。テキスト・図の伝え方についてはこれで一区切り。画面全体の導線・
   ユーザビリティは別観点として下記#15で新設した。** ユーザーから
...
11. **スプリント／ダンシングのモデルを描き直す**（`components/sprites/pixelBike.jsx`）。
   ...
   **【2026-08・凍結】以下は全て試して失敗した。同じ道を再び辿らないこと。**
   - **(a) AIに12コマを新規に描かせる発注仕様書** … ユーザー却下。「AIがデザインを理解して
     いないので、全く新しいものを出力しようとすると形が崩れる」。仕様書
     （BIKE_SPRITE_SPEC.md / make_sprite_spec.py）は削除済み。**検査ツール
     `tools/bike_sprites.py` だけは残してある**（向き・接地・A/B整合の検査規約は
     スプライトの出所を問わず有効なため。CANVAS/CONTENT定数は破棄した仕様の数値なので
     新データに合わせて更新してから使う）。
   - **(b) 行の挿入・削除で姿勢を変える** … 胴に行を挿入すると胴だけが伸びてプロポーション
     破綻。脚の領域に挿入すると**同じ行にある車輪まで複製されて縦に潰れる**。この絵は選手と
     自転車が同じ行に絡み合っているため、行単位の操作では原理的に解決できないと判明。
   - **(c) 選手/自転車のレイヤー分離＋腕脚の手続き的な描き直し** … 分離自体は成功した
     （「行20より上は全て選手」「以下は肌色＝手脚」「胴から連結するジャージ」で選手284px/
     自転車577pxに分離）。しかし持ち上げた後に腕脚を先細りの線で繋ぎ直すと、**腕が真っ直ぐな
     棒、脚が塊**になり見るに耐えない。線を細くしても改善せず。1マス2〜3pxの解像度で
     説得力のある手脚を描くには、陰影とシルエットの作り込みという「絵の技能」が要る。
   - **結論**：幾何変形でも手続き的描画でも品質は出せない。**現行の絵はユーザーの手描き
     参考画像から抽出して作られており、その方法だけが実績のある唯一の道。** 再開する際は
     dancing/sprintの6コマ分の参考画像を用意してもらい、CLAUDE.md §6に従って抽出する。
12. ✅ 最終スプリント演出のマーカー位置を直す（完了・上記「§20」参照。★・水色の丸を廃止し
   引き出し線＋名前タグ方式へ統一、俯瞰マップとも統一）。
13. **最終スプリントで、ゴールしてしばらく経ってから通常姿勢(normal)に戻ってほしい**
   （2026-08指摘）。現状は`domain/shared/sprintPosture.js`の`sprintPosture(rem, isDancer)`が
   `rem <= 0.05`でnormalへ切り替えており、ユーザーの見た目には**ゴール直前でスプリント
   姿勢が解除される**ように見えることがある、との指摘。しきい値`rem`の基準や、演出内の
   カメラ・selfのvtとの同期タイミングを見直す必要がある（未着手）。
14. **シーズンモードで「引き抜き」「対談申し込み」系のイベント頻度が高くしんどい
   （特に序盤）**（2026-08指摘）。「最初の2年くらいは無しでいい」という要望。
   実装調査済み：`controllers/season/month.js`の`mlAdvanceMonth`相当箇所で
   **被引き抜き（`makePoachOffer`）が年数を問わず毎月16%の確率**で発生している
   （`month !== 0 && Math.random() < 0.16`）。「対談申し込み」に該当しそうなのは
   同ファイルの**移籍志願**（`requester`、ベンチ4ヶ月以上・能力55以上の選手が退団を
   申し出る、月25%）が近いと推測されるが、ユーザーの言う「対談」がこれと一致するかは
   要確認。対応案：`s.year <= 2`の間はこれらの発生確率を0にする、または大幅に下げる
   （未着手・要ユーザー確認のうえ着手）。
15. ✅ **導線（ナビゲーション）とユーザビリティ全般の作り込み**（完了・上記「§24」参照）。
   「マイライフで次に何をすべきか分かりづらい。ボタンがあまりにも多く、色の統一性や
   位置のまとまりなどもないため、動線が全くわからない。神は細部に宿るため、とことん
   ユーザビリティを考えて欲しい」。**#9（UIテキスト・ビジュアル）とは別観点**：#9は
   個々のテキスト・図の伝え方の話だったのに対し、これは**画面全体の情報設計・視線誘導・
   ボタン配置のルール化**の話。指摘された具体的な問題：
   - **「次に何をすべきか」が伝わらない**：マイライフ本編（`mylife/hub.jsx`）は
     練習メニュー・レース作戦・専門トレーニング・スポンサーの仕事・出場ボタン等が
     並列に並んでおり、今その選手にとって何が最優先の行動かをUI側が示していない。
   - **ボタンの数が多い**：1画面に縦積みされるボタン数そのものを見直す必要がある
     （優先度に応じて折りたたむ、頻度の低い操作を奥に隠す等）。
   - **色の統一性がない**：ボタンごとに個別の色（`C.sub`／`C.blue`／`"#e8a13c"`／`C.green`等）
     が場当たり的に割り当てられており、色に一貫した意味（例：進行系＝黄、任意アクション＝
     アウトライン、破壊的操作＝赤）を持たせられていない可能性が高い。
   - **位置のまとまりがない**：関連するボタン同士が近接していなかったり、逆に無関係な
     ボタンが同じグループに見えたりする配置になっている疑い。
   **着手時の注意**：#9と同様「1画面直して終わりにしない」。マイライフ本編がまず槍玉に
   挙がっているが、これは画面全体のデザイン言語（ボタンの色分けルール・優先度に応じた
   視覚的階層・次アクションの強調）を新たに定義する仕事なので、着手前にCLAUDE.md §1に
   従い設計図をユーザーに提示して合意を得ること（未着手）。

   **【重要】「ごちゃつき」の本体は#9ではなくこちら（実測値・2026-08）**：第2波の完了後に
   ユーザーから「本当にテキスト面終わりかな？もうごちゃついている点はない？」と問われ、
   **レンダリング結果の密度を実測**したところ、#9で潰したのは「1文が長い」という症状に
   すぎず、「断片が多すぎる」という本体が手つかずで残っていることが判明した。
   マイライフ本編（毎月見る画面）の実測：
   - **末端テキスト要素 109個**。うち**11px以下が69個（63%）**、10px以下が23個。
   - ページ高さ1830px（スマホ画面3面ぶん）。
   - 縦200pxごとの断片数は `11 / 23 / 16 / 7 / 9 / 17 / 7 / 11 / 8` で、
     **選手カード上部（y=200〜400）に23断片が集中**している。
   - ただし10px以下23件の内訳を見ると**14件はレーダーチャートの軸ラベル**（平坦・登坂・
     加速力等）で、これは図の構成要素なので小さくて当然。**「小さい文字」そのものが
     悪いのではなく、整理されないまま断片が並んでいることが問題**という切り分けになる。
   したがって「何を1画面に載せるか／何を折りたたむか／どの順で見せるか」という
   **情報アーキテクチャの問題**であり、言い回しの修正（#9）では解けない。密度と導線は
   本項#15でまとめて扱う（ユーザー合意済み）。着手時は上記の実測値を before として、
   改善後に同じ計測スクリプトで after を取り、断片数がどれだけ減ったかで効果を検証すること
   （計測方法：末端テキストノードを走査し、可視要素の`fontSize`と文字数・縦位置を集計）。

16. **能力上限の収束問題（極めると全員同じステータスになる）**（2026-08指摘）。
   「極めると全員同じステータス（全ステータスカンスト、差があるとしてもソフトキャップの
   せいで2、3ぐらいしか最高ステータスと最低ステータスに差がつかない）になるという問題が
   ある」。§26のUI統合作業中に調査し、実際にコードレベルで裏付けが取れた（未着手・
   バランス変更を伴う大仕事のため別途設計が必要とユーザーも合意済み）：
   - `DIFFICULTIES.growthCap`（シーズン）は**難易度だけで完全固定**（88/94/102/112）。
     選手ごとの資質による差は一切ない。
   - `mlGrowthCap(year, player, ml)`（マイライフ）は
     `90 + 経過年数(最大+20) + 実績ボーナス(最大+10前後×難易度係数) + talentCap(配合限定・
     通常0)`。`growthPow`（成長力S〜C、デビュー時に決まる素質）は**上限到達スピードを
     変えるだけで、最終的な天井そのものには一切影響しない**。
   - つまり「同じ脚質・同じ難易度」の中では、スカウト時の素質評価（成長力・初期能力の
     ばらつき）はキャリアを積むほど意味を失い、最終的にほぼ同じ天井（`growthFactor`の
     ソフトキャップにより天井を多少超えることも可能だが、その伸びやすさを左右する
     `breakthrough`も生成時base44〜58の狭いレンジ）に収束する構造になっている。
   - 唯一の恒久的な差別化要因は`talentCap`（配合限定・血統因子由来）だが、非配合の
     デビューキャラでは0のままなので効かない。
   **設計が必要な論点（未着手・次回セッションで着手前にCLAUDE.md §1の設計図を提示すること）**：
   成長力（growthPow）や初期能力ロールを、伸びる速さだけでなく**最終到達点そのもの**にも
   持続的に反映させるか（例：growthPowを`mlGrowthCap`/`DIFFICULTIES.growthCap`の加点要素に
   組み込む）。あるいは`breakthrough`のレンジを広げて「上限を超えてどこまで伸ばせるか」の
   個体差を今より大きくするか。シーズンモード側は選手ごとの差別化要素が現状皆無なので、
   マイライフ以上に設計の手を入れる必要がある。
