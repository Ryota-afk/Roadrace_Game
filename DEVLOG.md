...
- **Step13第3弾（敷地画面：BaseView。`FinalSprintCinematic`の投影・スプライト資産を流用）**：
  Sonnetで実装。4波構成の③に着手。`components/RaceView.jsx`の`IsoRider`（自転車＋選手の
  ドット絵スプライト。従来は`FinalSprintCinematic`専用の非export関数だった）を`export`化し、
  `riderWander`（決定論的な待機ふらつき）・`CAP_COLORS`とあわせて再利用した。②と同じく、
  BaseView自体は`main.jsx`へ配線せず単体で完成させて休眠させ、④の一括カットオーバーで
  メニューシェルと同時に本番導線として登場させる方針を踏襲。
  **分割**：投影・配置計算は`domain/season/baseViewLayout.js`（`isoProject`＝2:1
  ディメトリック投影、`loopPathPoint`＝矩形の周回路上の位置、`riderLoopPoint`＝選手ごとに
  `riderHash01`で初期位相をずらした周回位置、`levelScale`＝施設Lv(0〜max)を表示スケール
  (0.35〜1.0)へ変換、`buildingLevels(g)`＝シーズン状態から5棟のLvを導出する読み取り専用関数、
  `isPathTile`＝地面タイルが園路かどうかの判定）に集約し、JSXを一切持たない
  純関数群としてNode単体テスト可能にした（Step7の`domain/`層の設計原則をそのまま踏襲）。
  建物の配置・寸法・配色は`data/baseViewBuildings.js`（`BASE_VIEW_BUILDINGS`。固定レイアウトで
  座標をセーブに持つ必要がない）に切り出し、見た目は`components/base/BaseView.jsx`
  （地面タイル・3面ボックスの建物・周回する選手を合成し、`sortY`で奥行きソートして描画順を
  決める提示コンポーネント）。「メニューを開くと世界が停止する」体験は、経過秒数を
  requestAnimationFrameのタイムスタンプ差分で積算する`useElapsedSeconds(paused)`フックで
  実現した（`paused`中は積算そのものを止めるため、再開時に経過時間が一気に進んで選手が
  ワープする事故を防いでいる）。
  **実機検証で発見した見た目の不具合と修正**：初回実装の建物寸法（`hw`0.55〜0.9・`hl`0.4〜0.55・
  高さ13〜24）は480×300のキャンバスに対して小さすぎ、実機スクリーンショットで確認したところ
  5棟中4棟が「立体の建物」ではなく「旗」のように見えるという不具合があった。原因は寸法不足
  そのものだった（施設Lvによるスケーリング自体は正しく機能していた）ため、全棟の寸法・高さを
  約1.5〜2倍に拡大（`hw`0.75〜1.15・`hl`0.55〜0.75・高さ22〜36）し、周回路半径・地面タイル
  範囲も合わせて拡張して再検証し、5棟とも壁面・屋根が視認できる立体的な見た目に改善した
  ことをスクリーンショットで確認した。ただし小さい画面では依然として「旗」に近い印象が残る
  棟があり、これは寸法の不足ではなく2:1ディメトリック投影特有の「奥行き軸(w)と横軸(l)の
  投影係数の差で片側の壁面が幾何学的に狭くなる」性質によるもの（Node単体テストで座標を
  検算し、バグではなく意図通りの投影結果と確認済み）。**視覚的な磨き込みは意図的にスコープ外**
  とし、当初の4波計画で明示していた「Wave D：磨き込み」へ先送りする（今回は「レイアウト計算が
  正しく・施設Lvに応じて建物がスケールする・選手が周回する」という機能面の土台を固めることを
  優先した）。
  **検証**：`domain/season/baseViewLayout.js`の座標変換・周回路・レベルスケール・
  レベル導出・園路判定・建物データの健全性（重複key無し・footprint非重複・
  `buildingLevels()`のキーとの整合）を検証するNode単体テスト46ケースを新規作成し全PASS。
  `IsoRider`のexport化は`components/RaceView.jsx`という実運用中のファイルへの変更のため、
  グランツール回帰・mylife連打防止回帰・mylifeセーブ注入という実際にレース描画を経由する
  Playwrightスイートを再実行し無傷を確認（この変更は`export`キーワードの追加のみで、
  ファイル内からの呼び出し方は一切変えていないため理論的にも安全だが、実測でも確認した）。
  一時配線での実機確認は、SVG描画・建物ラベル表示・polygon数・時間経過での選手移動・
  実行時エラー0の計6項目で実施し全PASS、スクリーンショット2枚（同一状態を2秒間隔で撮影）で
  選手が周回路上を実際に移動していることを目視確認した。確認完了後`main.jsx`は
  `git checkout`で完全に元の状態へ戻し、コミットには一切含めていない
...
**ネクストアクション候補（メモ・未着手）**：`data/seasonMenu.js`の6大ジャンルは現状ほぼ全て
「大ジャンル＝小ジャンル1個」の1:1対応（`misc`のみ4個）。今後、各大ジャンルの中身をさらに
小ジャンルへ細分化・追加して深掘りする余地がある（例：🚴選手を「練習指定／主将・ケミストリー／
ユース・配合」等へ分割、🛒市場を「スカウト／FA・引き抜き／トレード／パーツ・消耗品」等へ分割）。
現状は第1弾で分解した6セクション関数をそのまま1個の小ジャンルとして繋いだだけなので、
分割自体は各セクション関数の中身（JSX）をさらに機械分解する作業になる見込み。

- **Wave D（BaseViewの視覚的な磨き込み・完了）**：Opusで設計、Sonnetで実装。ユーザーから
  「敷地画面が建物に見えないし何が何だかわからない、参考画像（カイロソフト系4枚）のように
  してほしい。建物と練習コースがあるといい」と要望を受けて着手。
  **診断で判明した実バグ**：`IsoBuilding`が描いていた壁2面は、実はカメラから**見えない奥側**
  の面だった。頂点N（画面Y最小）に接する2面を壁として描いていたが、固定カメラ
  （`Px=24,Py=12,Lx=22,Ly=-11`）ではhw>hlの建物は必ずS（画面Y最大＝カメラに最も近い頂点）に
  接する2面が可視面になる。奥側の面は屋根の菱形にほぼ隠れるため「旗」に見えていた
  （Step13第3弾では「投影の性質上片側の壁が幾何学的に狭くなる」という別の理由で片付けていたが、
  実際の主因はこちらだった）。
  **設計判断（ユーザー選択、3問）**：①メタファーは「外観：敷地を見下ろす」（インテリア断面では
  なく既存のIsoRider・投影計算をそのまま活かせる）。②スコープは「フル：建物＋コース＋地面＋
  小物」を一括実施。③季節演出（g.month連動）も導入。
  **可視面選択の一般化**：`domain/season/baseViewLayout.js`に`visibleFacePair(corners)`を新設。
  4頂点(N/E/S/W)のうちscreen Y最大の頂点を機械的に求め、その両隣を可視面のペアとして返す
  （left/rightはscreen x座標の大小で決める）。特定の頂点を決め打ちしないため、footprintの
  縦横比が変わっても常に正しい面を選ぶ（Node単体テストで別アスペクト比のfootprintでは
  実際にfrontがS以外になることを確認済み。ただし実際の5棟はhw>hl固定のため常にSになる）。
  **建物の表現方法を変更**：旧来は施設Lvに応じて`levelScale(0.35〜1.0)`で建物全体（footprintごと）
  を縮小していたが、footprintが動くと窓・扉の配置がその都度歪む上、低Lvほど豆粒になり
  逆効果だった。footprintは固定のまま`buildingFloors(level,max)`で階数(1〜3F)を導出し、
  高さ(`floors×floorHeight`)だけが変わる方式に変更した（クラブハウスは`levelKey=classIdx`の
  ため、B1/A/PROがそのまま1/2/3階建てに対応する仕掛け）。
  **建物のディテール追加**（`components/base/IsoBuilding.jsx`新設）：`wallPoint()`
  （壁4頂点のバイリニア補間）を使い、各可視面に階数分の窓格子（点灯/消灯は建物key+行+列の
  `riderHash01`で決定論的に配分）・入口の扉・壁と屋根の境のパラペット帯・接地影・3階建て限定の
  屋上アンテナ+旗・棟ごとの看板アイコン（🏠💪🔧⚕🔍）を追加。
  **周回路をリボンとして描画**（`components/base/Track.jsx`新設）：旧来は矩形の周回路を
  `t∈[0,1)`の等時間4分割で辿っており（辺の長さに関わらず各辺が必ずt=0.25分）、かつ
  地面タイルの色をわずかに変えるだけでコース自体を描いていなかった。`roundedLoopPoint`
  （弧長比例＝等速のパラメータ化、4隅を`cornerR`で丸めた角丸オーバル。`cornerR=0`なら旧矩形と
  一致）と`trackRibbon`（中心線の接線から法線オフセットで外側/内側エッジを求め、原点からの
  距離で外側/内側を機械判定）を新設し、`<path fill-rule="evenodd">`で中抜きのリボンとして
  描画、白の破線センターライン・スタート帯を追加した。選手も同じ`roundedLoopPoint`で
  周回するため、コースの見た目と選手の走行位置が常に一致する。
  **地面ゾーニング**（`components/base/Ground.jsx`新設）：`groundZone(w,l,...)`で
  infield（コース内側の芝）／plaza（コースのすぐ外の舗装）／outer（外周の芝）の3ゾーンに
  分け、季節パレットで配色（旧`isPathTile`は廃止）。
  **構図の変更**：建物5棟を「画面奥に一列（screen Y一定になるようw,lを選定：
  `dl=(12/11)dw`方向に動くとYが変わらない）」に再配置し、周回路を主役に据えた
  （旧レイアウトは4棟が周回路を東西南北から囲む配置で密度が低かった）。
  **季節演出**：`seasonOf(month)`（`MONTHS`のindexを3ヶ月ずつ春夏秋冬に分割）で
  `data/baseViewBuildings.js`の`BASE_VIEW_SEASON_PALETTE`から空・芝・舗装・並木・雪の配色を
  切り替え。冬のみ木に雪の帽子・屋根に薄い雪色を乗せる。
  **小物の充填**（`components/base/Props.jsx`新設）：木10本・ベンチ・街灯・自転車ラック・
  チームカーを固定配置し、建物・選手と同じ奥行きソート(sortY)に混ぜて密度不足を解消。
  **見つけて都度直した実装ミス**：①`IsoBuilding`の投影定数をコンポーネント内にハードコード
  しかけたが、`BaseView.jsx`と二重管理になるため急遽`proj`をpropsで受け取る形に修正。
  ②`BaseView.jsx`のカメラ中心`cy0`を建物配置の見直しのついでに130→78へ変更しかけたが、
  地面・周回路・小物の座標はすべて`cy0=130`前提で算出済みだったため、変更すると全体が
  上に52px以上ズレて破綻するところだった（実機確認前に気付いて130に差し戻した）。
  ③看板アイコンの位置に建物footprintの「front頂点」（S、footprintの最も右寄りの点）の
  x座標をそのまま使っていたため、アイコンが建物の中心からずれて表示されていた
  →`isoProject(b.w,b.l,...)`で求めた真の中心のx座標に修正。
  **検証**：Node単体テストを新規58ケース（`visibleFacePair`・`wallPoint`・`buildingFloors`の
  単調性・`seasonOf`の12ヶ月網羅・`roundedLoopPoint`の閉合と連続性・`trackRibbon`の外側/内側
  判定の一貫性）＋データ健全性49ケース（hw>hl・footprintがground範囲に収まる・季節パレット4種
  の必須キー等）を追加、全PASS。Playwrightは施設Lv最大(PRO/春)・Lv0(B1/冬)・中間(A/秋)・
  夏の時間経過の4パターンでBaseView描画・polygon数・実行時エラー0を確認し、既存の
  `step13_cutover_check*`（メニュー動線・21項目）と`step13_menunav_test`（43項目）を
  再実行して無傷を確認。スクリーンショットで実際に「窓が並ぶ3D建物」「白破線＋スタート帯の
  ある灰色のオーバルコース」「季節ごとの配色」「木・ベンチ・街灯・チームカーが周囲を埋める」
  ことを目視確認済み。

- **Wave D2（カイロソフト準拠の視覚再設計・完了）**：Opusで診断/設計、Sonnetで実装。
  Wave D完了後にユーザーから「窓があっても微妙。建物が菱形の謎物体。選手がずっと右を向いていて
  進行方向と合わない。地面の謎の市松模様。木のデザインが変。建物その他の斜めの角度が変。
  スマホだと画面がかなり余る」と具体的な指摘を受けて着手。
  **実機計測で切り分けた**：iPhone相当(390x844)で描画してSVGの実寸を測ったところ、
  伸縮倍率は1.0000（＝preserveAspectRatioによる歪みは起きていない）だが、
  **SVGが画面高の26.8%（226px/844px）しか占めていない**ことを数値で確認。角度の違和感は
  歪みではなく別の原因、余白は構造の問題と確定させてから設計に入った。
  **根因1：footprintの定義がそもそも非アイソメトリックだった**（最大の発見）。
  Wave Dまでの`isoBoxFaces`は `N=(-hw,0) / E=(0,hl) / S=(hw,0) / W=(0,-hl)` という
  **world空間で45°回転した菱形**をfootprintにしていた。これを2:1投影すると
  **画面軸に平行な長方形**になり（hw=hlのとき完全な長方形）、可視2面の画面上の幅が
  40.8px対7.4pxと極端に非対称になる。これがユーザー指摘「斜めの角度が変」の直接原因だった。
  footprintを **world軸に平行な矩形**（w∈[w±hw], l∈[l±hl]）へ改めると投影後は必ず正しい菱形になり、
  hw=hlなら可視2面の幅も一致する。全建物のhw/hlを揃え（0.85、クラブハウスのみ0.98）、
  この不変条件をNode単体テストで機械的に固定した。
  **根因2：投影軸が左右非対称**（`Px=24,Py=12,Lx=22,Ly=-11` → w軸長26.83 / l軸長24.6で9%ずれ）。
  `Px=Lx=26, Py=-Ly=13` に揃えて正しい2:1ディメトリックにした。
  **根因3：屋根が主役になっていた**。壁の高さ42pxに対し屋根の幅86pxで、原色の平面菱形が
  建物を「色板」に見せていた → 階高を10-14px→26pxに引き上げて壁を主役にし、屋根は
  彩度を落とした暗色＋軒の張り出し（footprintの1.16倍）＋軒の厚み（影）＋屋上パラペットに変更。
  窓もセルの約4割まで小さくし暗いガラス色（点灯窓は22%だけ）にして、壁が「窓のブロック格子」に
  見えないようにした。正面に看板帯＋機能アイコン（🏠💪🔧⚕🔍）を追加。
  **根因4：地面の市松模様**。芝を1タイルおきに明暗交互で塗っていたためチェス盤に見えていた
  → 芝はviewBox全面を覆う単色の矩形1枚にし、装飾（濃淡パッチ・草むら）を決定論的な散布点として
  点在させる方式へ。舗装プラザも1枚の大きなポリゴン＋敷石の目地線に変更。
  副次的にポリゴン数が1500超→210前後へ激減した。
  **根因5：小物がアイソメ格子に乗っていなかった**。チームカー・ベンチ・ラックがスクリーン座標の
  軸平行な矩形で描かれていた → 建物と共通の`isoBoxFaces()`で描く`isoBox()`ヘルパを新設し全て置換。
  接地影も楕円からアイソメの菱形へ。木も「幹＋暗/中/明の3層樹冠＋輪郭線」の構造へ作り直した。
  **根因6：選手の向き固定**。`IsoRider`は常に右向きの固定スプライトのため周回路の左側の直線で
  逆走して見えていた → `riderFacesLeft()`（わずかに先の位置とのscreen x差で判定）を新設し、
  左進行時は `translate(2x,0) scale(-1,1)` で当該スプライトを鏡像反転する。
  **画面の余白**：キャンバスを480x300の横長→480x860の縦長にし、`preserveAspectRatio`を
  `"none"`（潜在的な歪みリスク）から`"xMidYMid slice"`（はみ出す方向を切り落として敷き詰める）へ変更。
  `makeWrap`に`opts.fill`を追加し、BaseView表示時のみ内側コンテナを縦フレックスにして
  残り高さを全て与えるようにした（他の全画面は`wrap(children)`のまま無変更）。
  結果、SVGの画面占有率は **26.8% → 82.0%** になった。
  なおキャンバス比は実機比(0.523)に寄せて0.558にしてある：`slice`はviewBox比が実機比から
  離れるほど左右が欠けるため、初版の480x720では左右計103pxが切れて端の建物が半分見えなく
  なっていた（0.558なら計30px程度に収まる）。
  **ついでの死にコード除去**：Step13第4弾で旧5タブUIを撤去して以来`withNav`を渡す呼び出し元が
  無くなり、`components/chrome.jsx`の`SeasonNav`はどこからも描画されない死にコードになっていた
  （CLAUDE.md §5）。これを削除し、空いた`wrap`の第2引数を上記の`opts`に作り替えた。
  **実装中に見つけて直したもの**：①奥の並木を建物列のすぐ奥かつ建物と同じ間隔で置いたため、
  各棟の真後ろから樹冠が顔を出して「屋根の上に木が生えている」ように見えた（画面最上部
  d=-44へ移動して解消）。②チームカーのキャビンを`transform="translate(0,-9)"`で持ち上げていて
  アイソメの足元がズレていた（「地面から生えたより高く細い箱」として高さで表現する方式へ変更）。
  **検証**：Node単体テストをdomain 75ケース・data 126ケースに拡充し全PASS。特に
  「hw=hlのとき可視2面の幅が一致する」「投影後が長方形ではなく菱形になっている」
  「5棟のscreen yが揃いxが等間隔」「プラザ上の小物が本当にプラザ内にある」「芝の木が舗装の上に
  生えていない」「奥の並木が建物列より十分奥」「選手の向きが1周の間に左右両方現れる」など、
  今回の視覚的な不具合をそのまま機械的な不変条件として固定した（回帰させないため）。
  Playwrightは施設Lv最大/Lv0/中間 × 春/冬/秋 と夏の時間経過で描画・実行時エラー0を確認し、
  既存の`step13_cutover_check`(18項目)・`step13_menunav_test`(43項目)も再実行して全PASS。

## 11. 敷地画面の「カットアウト部屋＋カメラ」化（2026-07・Step13 Wave E・進行中）

**背景**：Wave D2で外観の視覚語彙は正しくなったが、ユーザーから手描きスケッチとともに
「敷地全体を1枚のアイソメ平面にして、屋外のコースと**床＋奥2壁だけのカットアウトの部屋**を
同居させたい。ズームイン/アウトと視点移動ができれば横長のフィールドでも画面を埋め尽くせる
のでは」という提案を受けた。3Dの箱としての建物は廃止し、上から中が見える部屋にする。

**設計判断（ユーザー選択）**：①ズームアウトの下限は「全体が見えるまで引ける」（contain）。
余白はカメラ変換の外側に置いた芝の背景が埋めるので黒い余白は出ない。②部屋タップで対応する
メニューセクションを開く（建物がそのまま行動への導線になる）。③5棟すべてカットアウト化
（外観の3D箱は使わなくなる。ただし箱の描画自体は小物＝車・ベンチ等で引き続き使う）。

**壁を描く2辺は機械的に決まる**：部屋を w∈[w0,w1], l∈[l0,l1] としたとき、カメラから最も遠い
頂点は (w0, l1)（screen y 最小）。そこに接する2辺 —— w=w0 の辺と l=l1 の辺 —— が奥側の壁になる
（スケッチの「∧」型と一致）。手前2辺は開いたままなので中が見える。

**段階**：E-1 カメラ（ズーム/パン）→ E-2 建物のカットアウト部屋化 → E-3 什器（施設Lv連動）
→ E-4 人＋吹き出し。

- **Wave E-1（カメラ：ズーム/パン・完了）**：座標系を3層に整理した——world座標(w,l)／
  scene座標(isoProjectの出力・カメラの影響を受けない固定座標)／view座標(実ピクセル)。
  カメラはscene→viewの相似変換だけを担うため、既存の投影・レイアウト計算
  （`baseViewLayout.js`）を一切変更せずに後付けできた。
  **`domain/season/camera.js`（純粋計算）**：`sceneContentBounds`（描画物が占めるscene範囲）・
  `fitScale`/`coverScale`・`clampCam`・`cameraTransform`・`zoomAbout`・`viewToScene`。
  **`hooks/useIsoCamera.js`**：1本指ドラッグ＝パン／2本指ピンチ＝ズーム／ホイール＝
  カーソル位置基準のズーム。移動量がしきい値未満なら「タップ」として`onTap`にscene座標を
  通知する（E-2の部屋タップで使う）。
  **境界を「world矩形」ではなく「実際に描く物」から求めるのが要点**：world軸に平行な矩形は
  2:1投影すると必ず横2:縦1になるため、world矩形を境界にすると縦長スマホで上下が大きく余る。
  実コンテンツ（プラザ・周回路・建物・小物）の和集合は894x933とほぼ正方形で、縦長画面に収まる。
  **`preserveAspectRatio`への依存を解消**：ResizeObserverで描画領域の実ピクセルを測り
  `viewBox`をそれと一致させた（1 SVG単位=1 CSSピクセル）。拡縮は全てカメラが担うため
  `slice`による切り落としも歪みも原理的に起きなくなった（Wave D2で「viewBox比が実機比から
  離れると左右が欠ける」ことに悩んでいた問題が構造的に消えた）。
  **芝の下地をカメラ変換の外側へ**：ビューポート全面の`<rect>`にしたので、どこまで引いても
  必ず芝色で埋まり黒い余白が出ない（ユーザーの「埋め尽くしたい」要求をこれで満たす）。
  **実装中に見つけて直したもの**：ズームアウト下限で内容が画面に収まらない不具合。
  `getBBox`で実測したところ描画内容が2384x1200あり、算出した境界894x933と食い違っていた。
  原因は芝の装飾（scatter）だけが`BASE_VIEW_GROUND`の広大なworld矩形（投影後2444x1222）に
  撒かれており境界計算に入っていなかったこと。装飾をカメラ境界内にクリップして解消した。
  **検証**：Node単体テスト34ケース新規（fit/coverの定義通りの性質、クランプの端条件、
  「内容が画面より小さい軸はカメラが中央固定」、**ズーム前後で指の下のscene座標が動かない**
  アンカーズームの不変条件、`cameraTransform`と`viewToScene`が互いに逆変換であること）。
  Playwright 12項目（viewBoxが実ピクセルと一致・＋/－/ホイール/ドラッグの各操作が
...
- **Wave E-2 redo（5棟→単一クラブハウス＋4持ち場への作り直し・完了）**：Opusで再診断・
  設計、Sonnetで実装。最初のWave E-2完了後、ユーザーから「五棟と言うより、大きな室内に
  それぞれ対応した場所(机とか)があるイメージ。レイアウトもめちゃくちゃ。右手前側が大きく
  室内、左奥側が大きく外(サーキットや練習設備など)」と指摘を受け、スケッチを再確認したところ
  読み違いが判明した。外周の大きな菱形1枚（敷地全体）を対角線で「奥から左寄り＝屋外
  （コースのループ）」「手前から右寄り＝屋内（壁のハッチングが∧型・扉・机・椅子・
  「室内」ラベル）」に2分割する構図であり、5棟の建物という前提そのものが誤りだった。
  **設計判断（ユーザー選択、2問）**：①クラブハウスは大きな屋内ルーム自体（4つの持ち場が
  その中にある）。②ローラー等の機材は屋内（トレーニングコーナー）。
  **データ構造の作り直し**：`BASE_VIEW_BUILDINGS`（5棟配列）を廃止し、`BASE_VIEW_CLUBHOUSE`
  （単一の大部屋。footprint w:5〜14, l:-3.5〜4.5）と`BASE_VIEW_STATIONS`（4持ち場：
  training/mechanic/medical/scout。クラブハウスのfootprint内の異なる位置に配置）へ分離。
  周回路は従来通りworld原点中心に据え置き、クラブハウスをそこから見て手前+右（w大きめ）へ
  配置するだけで、cutawayの奥壁/開放辺の判定（`backFacePair`。カメラから見て常に正しい
  向きになる）はそのまま使い回せた（part位置に依存しない設計だったため）。
  **持ち場の什器**：`components/base/Station.jsx`新設。ローラー台（低い台+ローラー2本）・
  作業台（天板+フレームの線+工具箱）・診察台（白いベッド+ワゴン）・スカウト用デスク
  （天板+ノートPC状の板+書類）の4種を、既存の`isoBoxFaces`を使った箱描画で作成。
  什器の種類・数を施設Lvに応じて増やすのはWave E-3で行う（現時点では固定1つ）。
  **タップの当たり判定**：`stationQuad(s, size, proj)`を新設（`roomFloorQuad`と同じ考え方で
  持ち場1つぶんの小さな四角形を返す）。持ち場を優先的に判定し、外れたら部屋全体の
  `roomFloorQuad`へフォールバック（＝床の何もない場所をタップするとクラブハウス扱い＝
  メニュー全体を開く）。`ROOM_SECTION_MAP`のキー構成は変えていない
  （training/mechanic/medical/scout→facility、clubhouse→null）ため`hub.jsx`は無変更で済んだ。
  **実装中に見つけて直した実バグ（2件）**：
  ①**什器が床に埋もれて見えない**：クラブハウス（部屋）と持ち場（什器）を独立した
  drawOrderエントリとして奥行きソート(sortY)していたところ、大部屋自身のsortY
  （footprintの最前面＝カメラに最も近い点）が、内側にマージンを取って置かれた
  什器のsortYより大きくなるため、不透明な床ポリゴンが什器より後に描かれて覆い隠して
  しまっていた（実機スクショで什器がほぼ透けて見えないことに気付いて発覚）。
  部屋＋全持ち場を1つのdrawOrderエントリにまとめ、内部で必ず「床→什器」の順に描く
  ことで解消した。
  ②**初期表示でクラブハウスかコースの片方が画面外になる**：Wave E-1では初期ズームを
  cover（画面を隙間なく覆う倍率）にしていたが、クラブハウス＋コースを横並びにした
  ことで内容の縦横比が約1.95（横長）になり、縦長スマホ（約0.52）のcoverでは横幅の
  大部分が画面外へ押し出され、初期表示ではどちらか一方しか見えない状態になっていた
  （タップ検証で「計算上のタップ座標がビューポート外」というエラーから発覚）。
  芝の下地がカメラの外側にあり倍率に関わらず必ず画面を埋める（Wave E-1）ため、
  初期表示をcoverからfit（全体が収まる倍率）へ変更しても黒い余白は出ない。
  最初から敷地全体を見せてから寄っていく体験に変更した。
  **検証**：Node単体テストを新規データ構造に合わせて全面更新（domain 58・data 96・
  room 37・camera-math 29ケース、計220ケース）、「持ち場の当たり判定同士が重ならない」
  「持ち場はクラブハウスのfloor内にある」「初期表示のfit/cover/上限の関係」等を機械的に
  固定。Playwrightは新規のタップ検証4項目（mechanic/scoutの持ち場タップ・クラブハウスの
  床タップ）とカメラ操作12項目を実機のtransform読み取りで再検証し、既存の
  `step13_cutover_check`(18)・`menunav`(43)も含め計297項目全PASS。スクリーンショットで
  「コースとクラブハウスが初期表示で同時に見える」「各持ち場に対応した什器がはっきり
  見える」ことを目視確認済み。
...
**DEVLOG.mdのサイズについて**：本エントリ追加時点で約145KB（CLAUDE.md §4の目安
「十数万バイト」を明確に超えている）。次のセッション冒頭、あるいは次の区切り
（Wave F-3完了時等）で、古いwave（Wave D／D2／E-1／E-2初回など）の本文を
「件名＋git logでの復元方法」の索引形式へ圧縮してよいかユーザーに確認すること。
