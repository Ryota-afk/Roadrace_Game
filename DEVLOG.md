...
**第3波（未着手）に残した項目**：
- ヘルプ画面（`mylife/help.jsx`・`season/hub/help.jsx`）の本文。ここは**説明が主目的の画面**
  なので長文自体は妥当だが、1画面あたりの分量と読ませ方（見出しの粒度・折りたたみ）は
  別途検討の余地がある。
- `OVR`という略語。スポーツゲームの慣例で通りは良いが、初見に伝わるかは要判断のため保留。
- シーズンヘッダーのスポンサー行が1行に5項目（月給／ノルマ/違約金/指定レース済/見送り）を
  詰め込んでいる点。数値自体は運営判断に使うため削れず、見せ方の再設計が要る。
...
9. **ゲーム内の無駄なテキストを削り、「ゲームとして良いUI」に作り込む。**
   **【進捗】第1波（§22）・第2波（§23）を実施済み。第2波では全53画面ファイルを機械抽出で
   棚卸しし、フレーバー文を除く仕組み解説・開発語彙・保証文を一掃した。第3波（ヘルプ画面の
   読ませ方、`OVR`の是非、シーズンヘッダーのスポンサー行の再設計）は未着手。** ユーザーから
