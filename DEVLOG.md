...
- **統合直後の実機バグ2件の修正（完了）**：ユーザーが実機で発見した「①コースのカーブで
  自転車の向きが不自然」「②コースを外れて芝生を斜めに横切る」を修正した。
  **①向きの不一致**：当初「カーブ頂点で進行方向がほぼ水平になり、SE/NEの2方向しか
  用意していないため大きくジャンプする」という近似誤差の話だと考えたが、ユーザーから
  「原因はそこではない、赤丸のキャラは青丸のキャラと同じ向きにすれば自然になる」と
  明確な反例を提示され、再調査。原本の参考画像（矢印付きの1枚目）を見直すと、SEは
  既定で左向き(矢印↙)、NEは既定で右向き(矢印↗)に描かれており、互いに鏡像の関係にある
  1枚の絵ではなく独立した2枚の絵だと判明。`PixelBike`はSE/NE共通のflip条件
  （flip===trueで鏡像化）を適用していたため、NEは正しく表示される一方でSEは常に
  左右が逆になっていた。`dir==="SE"`のときだけ鏡像化の条件を反転させて対応した。
  BaseView上に進行方向を示す赤い矢印を一時的に重ね描きするデバッグ手法で、複数の
  カーブ地点で前輪・ハンドルの向きが矢印と一致することを目視確認してから本修正に
  至った（診断用コードはNode回帰確認後に削除）。
  **②コースを外れる**：`riderActivity.js`の「approach/depart」区間（周回⇔駐輪ラック間の
  移動）が、周回路上のどの地点からでも駐輪ラック（周回路の外側の角）へ直線で移動する
  実装だったため、周回路の反対側で周回を終えた選手は芝生を斜めに横切って見えていた
  （Wave F-3aからの既存動作・今回のドット絵化とは無関係）。区間を前半/後半の2段階に
  分割：前半は周回路パラメータ(t)を最短経路で補間して「周回路沿いに」ラック最寄り点まで
  進み、後半だけ最寄り点からラックへ短距離で直線移動する。ラック最寄り点は
  `nearestLoopT()`で周回路上を200分割して数値探索する（モジュール内の軽量ヘルパー、
  新規アセット不要）。Node側で全approach/depart区間678サンプルがコース帯または
  ラック付近3.5単位以内に収まることを確認（逸脱0件）。
  **検証**：既存Node全スイート（①②とも）が全PASS。本番ビルド成功。実機スクリーンショットで
  両方の修正を確認。

- **自転車の頭部クリップ修正＆ペダリングアニメーション追加（完了）**：
  **頭部クリップ**：ユーザーから「通常とダンシングの頭が切れている」と指摘。実データを
  確認したところ全6コマのグリッド行0に既にコンテンツがあり、bboxの上端ギリギリで
  ヘルメット上部を切っていたと判明。bbox上端に15px、他辺に8pxの余白を加えて再抽出した。
  ところが再指摘があり調べ直すと、**データではなく確認用プレビューのSVG表示範囲
  （viewBox）が低すぎたのが真因**だった（「通常」「ダンシング」は背が高く(50〜51マス)
  収まらず、前傾姿勢で背の低い「スプリント」(44マス)だけ偶然収まっていた）。
  プレビューのviewBoxを拡げて6コマとも収まることを確認してから再送した。
  **ペダリングアニメーション**：クランク位相違いの参考画像をユーザーが追加で用意
  （同一キャラ・同一構図・落ち影/ラベル無しの同条件）。既存と同じ抽出パイプラインで
  6コマ（`BIKE_B`）を取得し、`PixelBike`に`t`/`phase`引数を追加して
  `Math.floor((t+phase)*2.2)%2`でBIKE⇔BIKE_Bを交互切り替え（周期約0.9秒、歩行の
  WALK_A/WALK_B切り替えと同じ考え方）。`BaseView.jsx`から`t={elapsed} phase={item.phase}`
  を渡すよう変更。抽出時は最初から余白を確保し、頭部クリップの再発を防いだ。
  **検証**：既存Node全スイートが全PASS。本番ビルド成功。A/B両位相を並べたプレビューで
  頭部が枠内に収まること、脚とペダルの位相が実際に入れ替わっていることを確認。

- **ペダリング切り替え速度の高速化（完了）**：ユーザーから「切り替えはもっともっと
  早くていい」と指摘。`PixelBike`のBIKE⇔BIKE_B切り替え周期を約0.9秒(係数2.2)から
  約0.22秒(係数9.0、約4倍)へ変更。プレビューを120ms間隔で連続スクリーンショットし、
...
- **BIKE(A)/BIKE_BのnormalNE反転修正を取り消し（ユーザー指摘により revert）**：
  直後に着手した`normal_NE`の水平反転修正（コミットabcceeb）は、ユーザーから
  「今の修正全て元に戻して欲しい」と指摘を受け`git revert`で取り消した
  （コミットb507ad4）。ユーザー確認前に自分の判断でBIKE_B側まで反転対象を
  広げたことも含め、この時点のnormal_NEの向き判定については振り出しに戻っている。
  次に着手する際は、まずユーザーに実機のどの箇所がどう見えるか具体的に
  再確認してから、データ修正よりも先に原因を一つずつ切り分けること。

- **周回中の向き判定が横ゆらぎ(riderWander)と不整合だったバグの修正（完了）**：
  ユーザーから「NWに進もうとしている時にNE向きになる、逆(NE→NW)もある。ただし
  常にではない」と間欠的な現象を指摘された。SE/NEの意味を「方角」として再確認した
  上で、`activityDir`/`activityFacesLeft`はいずれも`riderActivityAt`が返す
  ゆらぎ抜きの位置だけで進行方向を判定している一方、`BaseView.jsx`は実際の描画位置
  にだけ`riderWander`の横ゆらぎ(周期的、係数0.10)を加えていたことに気づいた。
  Node上でシミュレーションした結果、周回中の全サンプル28887件中1294件(4.48%)で
  「向き判定用のflip」と「ゆらぎ込みの実際の画面移動方向」が食い違っており、しかも
  dir=NE時の食い違いが687件（NE⇔NW双方向、間欠的＝ゆらぎの位相次第）で発生して
  いることを確認し、報告内容と一致することを検証した。カーブ頂点付近など進行方向の
  画面x成分がもともと小さい区間では、ゆらぎの変化量が本来の移動量を上回って実際の
  見た目の動きが逆転しうるのが原因。
  **修正**：`riderWander`を`components/RaceView.jsx`（JSXファイル・view層）から
  `sim/race.js`（JSXを持たない下位層）へ移設し、`riderActivity.js`からdomain層の
  依存方向を保ったまま参照できるようにした。同ファイルに`activityWobble(rider,act,tSec)`
  を新設し、`activityFacesLeft`/`activityDir`の向き判定サンプリングにも同じゆらぎを
  適用。`BaseView.jsx`側の描画位置計算も同じ`activityWobble`を呼ぶよう統一し、
  同じ式を2箇所に別々に書く二重管理を解消した。
  **検証**：修正後は同じシミュレーションで食い違いが28887件中17件(0.06%)まで低下、
  残りは全て`|dx|<0.01`の静止判定しきい値付近の意図した挙動であることを確認。
  1人のライダーを200秒(7周相当)シミュレートしflip/dirのトグル回数が1周あたり
  4回・2回程度の妥当な頻度であること（新たな過剰なチラつきが発生していないこと）
  も確認。本番ビルド成功。

- **NE反転(flip)の再検証（コード上は問題なし・原因未特定のまま継続）**：ユーザーから
  再度「街灯の角でNE向きに見える、道はNW向きのはず」と指摘。実際の`activityDir`/
  `activityFacesLeft`をそのまま呼び、街灯(lamp1: w=-2.5,l=3.0)近傍のt=0.68〜0.83に
  選手を並べてdir/flipをラベル表示で直接確認したところ、該当区間は正しく
  `NE`+`flip=true`（＝NWを表す反転済みNE）と判定されていた。中心線を引いた比較でも
  反転自体は機能していることを確認。念のためdancing/sprint姿勢やBIKE_B(ペダル位相B)の
  NEの左右handedeness もBIKE(A)のnormal_NEと一致することを目視確認し、ここまでは
  ユーザー指摘の「反転できているキャラとできないキャラがいる」の原因を特定できて
  いない。次に着手する際は、複数選手が同時に見えるスクリーンショットで「反転できて
  いる個体」と「できていない個体」を直接指し示してもらい、姿勢/位置/タイミングの
  どれと相関するかをまず特定すること。

- **スタート/フィニッシュ帯をラック最寄り点に固定（完了）**：`Track.jsx`のスタート帯は
  周回路パラメータの`t=0`固定点（`roundedLoopPoint`の内部定義上たまたまそこが原点）に
  描かれており、ラックの位置とは無関係だった。ラックに最も近い周回路上の点を返す
  `nearestLoopT()`（元は`riderActivity.js`内のprivate関数）を、コース形状の計算として
  より適切な`baseViewLayout.js`へ移設・export し、`riderActivity.js`はそこから
  importする形に変更（ロジック自体は変更なし、置き場所の是正）。`Track`に`rack`propを
  追加し、`nearestLoopT(pathW,pathL,cornerR,rack)`で求めたtに一番近いリボンの
  サンプル点(`i0`)を使ってスタート帯の四角形を生成するよう変更。
  **検証**：ラック位置に赤い目印を重ねたプレビューで、白いスタート帯がラック直上の
  周回路上に描かれることを目視確認。`nearestLoopT`移設後もapproach/depart区間541
  サンプル全てがコース帯またはラック付近3.5単位以内に収まること（0件逸脱）を再確認。
  本番ビルド成功。

- **選手の周回スタート/フィニッシュ地点もラック最寄り点に固定（完了）＋depart末尾の
  ワープバグを誤って作り込み即修正**：帯だけでなく選手自身が周回をスタート/
  フィニッシュする地点もラック最寄り点(nT)に揃えたいという指摘。`riderActivityAt`の
  ride位置計算を、以前の「選手固有の位相(riderHash01(id,41))による継続クロック」
  （`riderLoopPoint`、tSecの連続関数で周回上のどの点から始まるか無関係）から、
  「このサイクルの開始時刻(tCycleStart)にnTを出発し、経過秒数×speedだけ進める」
  形(`loopT`)に変更。選手ごとにtCycleStart自体がriderHash01(id,53)でずれているため、
  この変更後も選手が団子状態になることはない。
  **作り込んだバグとその場で発覚・修正**：変更直後、depart区間の後半
  (`loopT(tNextRideStart)`)が次サイクルのtCycleStartを跨いで評価されるため、実際に
  次のride開始時に評価される位置（新方式では常にnT）とズレ、選手がワープして見える
  重大なバグを作り込んだ。Node上で「approach/depart中の隣接フレーム間移動量」を
  計測する回帰チェックを新設したところ最大5.9単位のジャンプを検出（通常は0.04単位
  程度のはずで明らかに異常）。原因はdepart後半の「周回上の未来位置へ先回りする」
  処理が新方式では無意味（rideは常にnTから始まるため）になっていたこと。depart区間を
  「ラック→nTへ全区間かけて直線移動」という単純な1区間に統合し解消。
  **検証**：修正後、隣接フレーム間移動量の最大値が0.14まで低下（正常範囲）。
  ride開始位置とnT点の誤差が1e-6以内であることを数値確認。approach/depart4023
  サンプル全件がコース帯またはラック付近3.5単位以内（0件逸脱）。本番ビルド成功。
  未使用になった`riderLoopPoint`の呼び出しと`tNextRideStart`変数を削除
  （`riderLoopPoint`/`riderFacesLeft`自体はbaseViewLayout.jsに残置＝呼び出し元ゼロの
  デッドコード候補。次回の構造整理の際に検討）。

- **「2周目以降に反転がバグる気がする」の調査（未特定・継続）**：ユーザーから
  「感覚的に2周目以降に反転(flip)がバグる」との指摘。`activityFacesLeft`の
  「画面移動量|dx|<0.01なら静止とみなす」しきい値処理を精査したところ、
  これは実際には「直前のflipを維持する」のではなく**常にflip=falseへ固定**する
  実装になっており（コメントの意図と実装がズレている）、dx がわずかに負のまま
  0.01を下回る瞬間だけflip=falseに強制されて食い違う、という有向性のある
  （常にflip=false側に偏る）微小な不一致を発見した。ただし該当区間の|trueDx|は
  最大でも0.0094（1周あたり2箇所、周回のうち一瞬だけ）と極めて小さく、
  「2周目から」という体感の再現には至っていない。この現象自体はlap数に関係なく
  毎周同じ場所で起きるため、ユーザー指摘の「2周目以降」という時間発展的な症状とは
  一致しない可能性が高い。次に着手する際は、具体的にどのタイミング・箇所で
  「反転がバグる」と感じたか（同じキャラをある程度の時間目で追い続けた上での指摘か、
  特定のコーナーか）を確認してから再調査すること。
...
**UI/メニュー構造**
1. メニュー画面で「ホームに戻る」選択肢を、大分類として即選択できるようにする。
2. 大分類の中の小分類が今はほぼ機能していない。例：「ショップ」(大分類)を
   「施設」「機材」「スタッフ」の3小分類へさらに細分化するなど、他の大分類も
   同様に細分化する。
...
- **「NEで反転できているキャラとできていないキャラがいる」の真因判明：
  dancing_NEの参考画像自体が通常と逆向きだった（完了）**：長期化していたNE反転の
  調査で、ユーザーから「絵柄の問題では絶対にない、直線で起きている、dancingのSE/NE
  立ち絵を見せて」「NE自体が左右逆じゃないか」と的確な指摘を連続でもらい、最終的に
  抽出元の参考画像そのものを再確認した。`39daf64b-IMG_2570.PNG`（クランク位相A）の
  「3.通常」のNEはハンドルが右側に描かれているのに対し、「4.ダンシング」のNEは
  ハンドルが左側に描かれており、**同じ「NE」ラベルなのに参考画像の段階で左右が
  逆**だった。`17e2ff28-IMG_0262.png`（クランク位相B）のダンシングNEも同様に左側
  だったため、抽出済みの`BIKE.dancing_NE`/`BIKE_B.dancing_NE`は両方とも通常と逆の
  handednessを機械的に忠実継承していたことになる。
  これまでの調査（コード上のdir/flip判定は全選手・全周回で一貫していることを
  Node実機シミュレーションで確認済み、dancingの絵柄が正面寄りで判別しにくいという
  説明）はいずれも的外れで、**参考画像の時点での左右不整合**が唯一かつ真の原因
  だった。ユーザーの「絵柄の問題ではない」「直線で起きている」という指摘が
  正しく、私の「dancing姿勢の絵柄的特徴」という説明は誤りだった。
  **修正**：`BIKE.dancing_NE`/`BIKE_B.dancing_NE`の2配列を水平反転し、通常(NE)と
  同じhandedness（無反転でハンドル右）に揃えた（`fix_dancing_ne_mirror.mjs`で
  機械的に実施、SE側・他姿勢は対象外）。なお参考画像1枚目「5.スプリント」は列の
  左右順がNE|SE（他行はSE|NE）と逆だったが、ラベルと絵の向きの対応自体は正しく
  （NE=ハンドル右, SE=ハンドル左のまま）、スプリントは現状BaseView側で未使用のため
  実害なし・対応不要と判断。
  **検証**：修正後、`BIKE(A) dancing NE flip=false`のハンドルが通常と同じ右側に
  来ることをレンダリングで確認。本番ビルド成功。

- **再発防止策をCLAUDE.mdに追記（完了）**：上記バグの根本原因（コードでなく参考
  画像自体のhandedness不整合）と、診断が長期化した原因（コード側のflipロジックを
  先に疑い、参考画像同士の直接比較を後回しにしたこと）を踏まえ、CLAUDE.mdに
  新設§6「ドット絵アセット抽出時の左右handedness整合性チェック」を追記した。
  要旨：①新しいポーズ/位相を抽出する際は確定済みの別ポーズと同じ参考画像上で
  ハンドル位置を目視突き合わせしてから確定する、②頭部/肌色ピクセル重心オフセット
  という数値ヒューリスティックは本プロジェクトで実績として信頼できないと判明済み
  （dancing_NEで誤判定した）、③「向きがおかしい」報告時はコードでなく参考画像側の
  不整合を最初に疑う、④ユーザーが「絵柄の問題ではない」と明言したらそれを実装上の
  言い訳で片付けない。

**DEVLOG.mdのサイズについて**：本エントリ追加時点で約139KB。CLAUDE.md §4の目安
「十数万バイト」に達しており、複数回にわたり提案を先送りしてきた。次の一区切り
（このセッションでのバグ修正が一段落した時点）で、実際にスリム化を実施するか
ユーザーに判断を仰ぐこと（先送りをこれ以上重ねない）。
