...
// 第80弾(devlog/wave75.md「第79・80弾」): 山岳ロードは素の走破タイムでCLMが最速なのに実測で
// 負けていた（原因B＝集団力学がまだCLMを削っていた）。climb/mtnを0.05→0.07へ強化する
// （regenを上げるとふるい落としが弱まり万能型有利に戻ると判明したため、regen側ではなく
// こちら側で強める）。0.08まで上げるとグラベルレース（climbを含む混成地形）でCLMが
// 過剰優遇されPUNを上回ったため、山岳ロードでCLMが最上位になり続ける最小限の値として0.07を
// 採用した（実測：山岳ロードCLM33〜56%で安定して最上位）。
export const TERRAIN_KEEP_TIGHTEN = { climb: 0.07, mtn: 0.07, hill: 0.035, flat: 0.015, sprint: 0.015, tt: 0.015 };
