...
export function BaseView({ g, paused, onRoomTap }) {
  ...
  // 第23弾: 前面帯ルール。クラブハウスの敷地矩形より南（l<frontL）・東（w>frontWMax）に
  ...
  // 第23弾追記・訂正：西（w<frontWMin）も一時的に対象へ加えたが誤りだった。前庭ベンチ
  // （bench-plaza0/1）はクラブハウスの西壁（Room.jsxがbackFacePairで不透明に描画する
  // 本物の奥壁）のすぐ外側にあり、そこで見えなくなるのはこのカメラ角度における正しい
  // 遮蔽であって不具合ではない（2026-08ユーザー指摘「それは壁の向こう側にあるので
  // 見えないだけ」）。南・東の2方向のみに戻した。詳細はdevlog/wave23.md参照。
