...
// 段階（金/銅）を実績（天井）×正規化スコア（現在値）で決める設計だったが、出荷直後の
// 計測でN=8では狙う/狙わないを判別できないと判明し**撤回した**（devlog/wave42.md末尾
// 「退行モデルの撤回」）。
//
// ⚠️このファイルの関数群は現在どこからも呼ばれていない（buildMyLifeSim.js・rider.jsxの
// 接続は撤回済み）。⚠️EXPOSURE_NORMの値は未較正——第41弾時点のレース選択（候補3本・
// 看板月は1本）で測ったもので、N=8では全バッジで無頓着/狙うの分布が重なることが実測済み
// （devlog/wave42.md「計測2」）。第43弾（出走計画・devlog/wave43.md）でレース選択の
// 操作量を増やした後、devlog/wave42.md末尾の計測1〜3を測り直し、N=8で分離するように
// なって初めてこのファイルの関数を効果へ再接続すること。分離しないうちは接続しない。
