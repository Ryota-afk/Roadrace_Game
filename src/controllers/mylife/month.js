...
export function mlAdvanceMonth(s, mode) {
  ...
  if (s.month === 11) {
    ...
    if (interest >= 0.8 && Math.random() < 0.6) {
      ...
      return finalizeYearEnd({
        ...
        races: mlGenRaceCandidates(s.year + 1, 0, classIdx, [s.raceFocus, s.raceFocus2]), sel: { ...s.sel, raceId: null },
  ...
  const base = {
    ...s, player, month, races: mlGenRaceCandidates(s.year, month, s.classIdx, [s.raceFocus, s.raceFocus2]), sel: { ...s.sel, raceId: null },
