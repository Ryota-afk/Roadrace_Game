...
export const BASE_VIEW_PROPS = {
  ...
  // 第23弾: flipはBASE_VIEW_OUTDOOR_SPOTSの対応するbench-*エントリと必ず同じ値にする
  ...
  // 第23弾確定（2026-08ユーザー指示）：flipは「ベンチ＋着席者の組」の向き。人物側は
  // この値をそのまま使い、ベンチ什器側はProps.jsxが`!flip`で描く（ベンチと人物SITの
  // スプライトは素の向きが互いに逆手性のため、逆符号で初めて背もたれと背中が噛み合う）。
  benches: [
...
// 第23弾: 選手の徒歩導線（walkIn/walkOut・approach/depart）が実際に目指す「乗降点」。
// 駐輪ラックの見た目のアンカー(BASE_VIEW_PROPS.bikeRack)とまったく同じ座標だと、選手が
// ラックの真上に立つ瞬間にsortYが同値になり奥行き比較が不定になる（2026-08ユーザー指摘
// 「自転車立てのレイヤーが選手より前に来ている」）。ラックより少し手前（l方向）にずらして
// 選手のsortYが常にラックを上回る＝常に選手がラックの手前に描かれるようにする。
export const BASE_VIEW_RACK_MOUNT = { w: 5.2, l: -5.65 };

// 第20弾: 小川。敷地左奥の縁（海）から入り、コース奥の芝帯を蛇行して裏の縁から海へ抜ける
// 中心線の折れ線（Catmull-Romで滑らかにして描く。domain/season/baseViewLayout.jsの
// streamCenterlinePts＋components/base/Stream.jsx）。両端を敷地の外に出すことで
// 「切りっぱなしの端が芝の上で終わって池が見切れて見える」問題を構造的になくす。
// コース帯とのクリアランスはtools/verify_baseview.mjsが機械検証する。
export const BASE_VIEW_STREAM = {
  points: [
    { w: -9.6, l: 4.0 }, { w: -8.2, l: 5.2 }, { w: -7.3, l: 6.5 },
    { w: -5.8, l: 7.35 }, { w: -4.2, l: 7.5 }, { w: -2.9, l: 8.2 }, { w: -1.9, l: 9.6 },
  ],
};
...
// 第21弾: 選手が屋外でも過ごす行き先（ベンチ・ジム・散歩）。devlog/wave21.md/wave23.mdの設計。
// ラックからの経路は[rack, ...waypoints, spot]で組み立てる
// （domain/season/riderActivity.jsのbuildOutdoorRoutes()）。waypointsは木・ランプ・
// ベンチ・コース帯・**クラブハウスの敷地矩形**とのクリアランス／非交差をtools/verify_baseview.mjs
// で機械検証してある。
// 第23弾で修正：前庭ベンチ2脚（bench-plaza0/1）の旧中継点(5.1,0.2)はクラブハウスの
// footprint（w:5〜14）の内側を通っており、選手が壁を貫通して見える不具合の原因だった
// （2026-08ユーザー指摘）。ベンチ自体はw=4.2〜4.3で既に敷地外（西側の前庭）にあるため、
// 中継点も建物の西側（w<5）だけを通るよう引き直した。bench0は東寄りコリドー(w=4.7)で
// 外灯(3.9,0.2)を避け、bench1はbench0との間隔（東側は壁までわずか0.7しか余裕が無く
// 回り込めない）を確保するため西寄りコリドー(w=2.9)を別に通す。
...
// 前庭ベンチ・ホームストレートベンチ・散歩はコース非横断、ジム・池のほとりはインフィールドに
// あり縦移動でコースを1回だけ横断する（検証器がこの横断回数を機械チェック）。
// flip: 「ベンチ＋着席者の組」の向き。人物（BaseView.jsxのBENCH_FLIP経由）はこの値を
// そのまま使い、ベンチ什器（Props.jsxのbenchNode）は`!flip`で描く。両スプライトの素の
// 向きが互いに逆手性のため、逆符号にして初めて背もたれと背中が正しく噛み合う
// （2026-08ユーザー指示で確定。BASE_VIEW_PROPS.benchesのflipと必ず同じ値にする）。
export const BASE_VIEW_OUTDOOR_SPOTS = [
  ...
  // 第23弾: 旧スロットは什器スプライトの外周（マットの外）に置かれており「ジムの前で
  // 立っている」ように見えていた（2026-08ユーザー指摘）。什器のドット絵データ
  // （sprites/pixelObjectData.jsのgym.rows）を実測し、接地マット（手前の菱形）の
  // 内側に収まる3点へ置き直した。
  { key: "gym", kind: "gym", pose: "stand", minLevel: 3,
    waypoints: [{ w: 2.8, l: -5.8 }, { w: -4.3, l: -5.8 }, { w: -4.3, l: -4.0 }],
    slots: [{ w: -4.35, l: -2.35 }, { w: -4.9, l: -2.6 }, { w: -4.85, l: -2.1 }] },
  // 散歩：3種類をローテーション（domain層のdestinationForが選手ごとに振り分ける）。
  // stroll＝西側の外周コリドー（既存・コース横断ゼロ）。
  { key: "stroll", kind: "stroll", pose: "walk",
    waypoints: [{ w: 2.8, l: -5.8 }, { w: -6.9, l: -5.8 }, { w: -8.5, l: -5.0 }],
    spot: { w: -8.5, l: -0.5 } },
  // stroll-pond＝湖のほとり（第23弾新設）。インフィールドへ縦移動で1回だけ横断して入る。
  { key: "stroll-pond", kind: "stroll", pose: "stand",
    waypoints: [{ w: 2.8, l: -5.8 }, { w: -2.9, l: -5.8 }, { w: -2.9, l: 2.8 }],
    spot: { w: 0.9, l: 3.0 } },
  // stroll-stream＝小川のほとり（第23弾新設）。西側コリドーをそのまま北上（横断ゼロ）。
  { key: "stroll-stream", kind: "stroll", pose: "stand",
    waypoints: [{ w: 2.8, l: -5.8 }, { w: -6.9, l: -5.8 }, { w: -8.7, l: -5.0 }, { w: -8.7, l: 3.0 }],
    spot: { w: -8.3, l: 4.2 } },
];
