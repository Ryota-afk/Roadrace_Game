...
- **逃げの速度を触る案は4回失敗しており、軸そのものが行き止まり**（第52弾＝定数／第53弾＝集団
  ...
  `brk≤0.18`は行き過ぎ（今度は「常に仕掛けるのが正解」になる）。
...
- **出走計画（第43弾）**：`ml.raceFocus`（`climb`/`hill`/`sprint`/`solo`/`null`）を宣言すると、
  **通常月の候補3本のうち1本が必ずその適性のレースになる**（確率ではなく確定仕様）。
  これが無いとプレイヤーは地形をほとんど選べない——コーステンプレは6種で山岳を含むのは
  2種、候補1本の月がクラス2以降で年6ヶ月あるため、狙っても年の半分は選択肢が無かった。
  効果は山岳が年3.4本→**8.1本**、独走が年1.2本→**7.0本**。**TT型は年1.2本しかTTを走れて
...
- **年齢による成長フェーズ `growthPhase`（`growth.js`・第38弾で再較正）**：係数は
  **`1.0/0.5/0.1` → `0.55/0.30/0.28`**、`abilities.js`の`GROWTH`各peak終端を**+4歳**。
  旧値は**30歳で全盛期0.5→衰え期0.1と1/10に急落する崖**で、これがカンストの真因だった
...
- **3分類（第47弾で実装済み・画面も3節に分かれている）**：
  ...
  - **体質16種**（体質系10＋悪特性6）＝**付け外し不可・枠を消費しない**。生まれつきで
...
###  レースsimを実測するときの必須手順（第49・51・55弾で確立・生きたルール）
...
4. **第56弾で修正済み（`MAX_TICKS`は2500→7000・凍結バグも同時に修正）**。旧バグの記録：
   ...
   2経路があり、**全テンプレートの13〜43%のレースで発生していた**（平坦でも13%）。
   修正はモード配布直後に「牽引者が居なければドラフト勢で最も脚が残る選手を立てる」
   ガードを1つ足しただけ（`ticks.js`）。`hold`・コースレコードは実測で不変（`devlog/wave56.md`）。

5. ⚠️**プレイヤーの取得は`entrants.find(e => e.isPlayerChar)`だけ。フォールバックを書かない**
   （第96弾）。`isPlayer`というフィールドは**entrantには存在しない**（チーム行・世界ランク行・
   `ridersUi`という別種のオブジェクトのフィールド）。`|| entrants[0]`を添えると`.find()`が
   失敗しても静かに**AI選手**を掴み続ける（プレイヤーは添字30前後）。⚠️**この1行のせいで
   「難易度つまみが逆転している」という弾を1本まるごと書いて撤回した。** 取れなければskipする。
6. ⚠️**出走レースは`careerRaces()`（`tools/_shared.mjs`）で組む**（第97弾）。
   `mlCreateChar()`が返す`races`は**その月の候補3件**であって年間日程ではない。これを使うと
   クラスや年次を変えても⚠️**新人1年目の2〜3レースを繰り返すだけ**になる
   （「円熟期」を一度も測っていなかった）。
7. ⚠️**着順は飽和する。Δ秒を必ず併記する**（第98弾）。能力100/PROでは1着に張り付くため、
   判断が94%の場面でタイムを動かしていても「Δ着順0＝効いていない」と読めてしまう。
   ⚠️**ただしΔ秒は万能ではない**——**顔ぶれ自体を変える計測**（難易度・本気度など）では、
   強い集団は速く走るので**後方に沈みながら絶対タイムは速くなる**。この場合は着順が正しい。
8. ⚠️**段階（キャリア時期・能力帯）を比べるなら、段階間で対応のある標本にする**（第99弾）。
   段階ごとに`mlCreateChar`を引き直すと⚠️**キャラの引きの差が段階差に化ける**。実測で
   同一条件がn=4で77%と98%、n=10でも14%と31%にぶれた。⚠️**同じ誤りを2回繰り返している。**
   キャラ集団を1度だけ作り、各段階へは能力だけスケールした複製を流すこと
   （`tools/intensity_probe.mjs`が実装例）。⚠️**同一実行内の条件比較（同じキャラ・同じレースを
   全条件へ流す）は信頼できるが、実行をまたいだ比較は信頼できない。**
9. ⚠️**`buildMyLifeSim`の同じレースを別パラメータで測るなら`seed`引数を渡す**（第99弾）。
   省略すると`Date.now()`由来の乱数列になり、⚠️**出走者の人数・チーム構成・当日の調子まで
   毎回入れ替わる**。同一seedなら顔ぶれ・年齢・成長タイプ・当日の調子が完全に一致し、
   `power`だけが動く（`newRider`内のrng消費量は`power`に依存しないため）。
10. ⚠️**AIの強さは`aiMul`と`abilCap`の両方で決まり、片方だけ動かしても効かない**（第99弾）。
   `newRider`が各能力を`abilCap`でクランプするため、上限を上げずに`aiMul`を**3倍**にしても
   表彰台率は91%→80%しか動かない。逆に⚠️**PRO帯のAI実力は平均約75で上限96に届いておらず、
   天井は当たっていない**ので`abilCap`だけ上げても何も起きない。
11. ⚠️**`tools/simcareer.mjs`の方策比較（n=8）で「どちらが得か」は判定できない**（第99弾）。
   総合力の実行間ブレが約8ポイントあるのに対し方策間の差は2.5ポイント。⚠️**3回試して3回とも
   ノイズに埋もれた。** n=40規模（実行3時間級）を用意できないなら、この方法での判定は
   再挑戦しないこと。
12. ⚠️**`tools/intensity_probe.mjs`は`ageWorldRosters()`を再現しない**（`worldRosters`が
   `mlCreateChar`の返す1年目のまま）。⚠️**「15年目」は15年目のレース日程と乱数シードであって、
   15年目のAIの強さではない。**
