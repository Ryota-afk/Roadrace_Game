...
4. ⚠️**`MAX_TICKS = 2500`は固定値**なのにレース長は地形で2198〜3275秒と違う。超過分は
   `finishTime = MAX_TICKS + (remain / lastDist)`という**外挿**で埋まる＝**着順がシミュレー
   ションではなく外挿式で決まっている**（ヒルクライムで未完走71%・山岳20%）。登坂系を測る
   ときはこの汚染を前提に読むこと。**未修正の既知バグ。**
