# 第84弾：チームTTのレース後処理の欠落を埋める

**状態**：**実装完了。設計どおり①（人気度・絆・成績台帳）を追加し、副次的に見つかった
2件の配線漏れ（絆のsim.ranked未対応／マイライフのチームTTで結束が無視されていた）も
同じ弾で修正した。**
...
## ①-4 スコープ外（今回は入れない。監督評価はDEVLOG「次のアクション」#29-aへ記載済み）
...
## 実装結果と計測（CLAUDE.md §10）

設計どおり1〜6を実装。`npx vite build` は警告なしで通過。

### ④ chemMul配線の実測——設計時の見積り（1〜2順位）を大きく超えた

`scratchpad/w84_chem_impact.mjs`（n=200・自チーム僚友3人固定・絆0 vs 絆満で比較）：

| 絆の状態 | 平均チーム順位（9チーム中） | 上位3入り |
|---|---|---|
| 絆0（chemMul=1.0） | 6.84 | 18/200（9%） |
| 絆満（chemMul=0.92） | 2.87 | 141/200（70.5%） |

設計時の見立てより効果が大きい（約4順位ぶん）。ただし調べた結果、これは**新たに
導入した数値ではない**——`finish.js:134`の`chemBonus = 1 + (1 - chemMul)`と、
chemMulの可動域`0.92〜1.0`は、**シーズン側の`CHEMISTRY_TIERS`（既存・
`data/progression.js:72-77`）と完全に同じ範囲**で、シーズンのチームTTでは
`buildSim.js:141`が最初から`chemTier.mul`を正しく渡して稼働していた。今回の
修正は「マイライフだけこの既存の計算式が`computeTeamTT(sim, 1)`のハードコードで
無効化されていた」導線を直しただけで、**式自体の強さはシーズン側で既に生きている
仕様と同一**。よって新規のバランス調整は行わず、これは「絆を育てる動機付けとして
機能する」という設計意図どおりの挙動と判断した。

### ⑤ riderStats配線の実測——AI側の欠落を確認

`scratchpad/w84_verify_riderstats.mjs`（実際のチームTT候補レースで`buildMyLifeSim`→
`mlUpdateRiderStats(..., { teamResult: true })`まで通す）：

- 参加AI総数（自分除く）：**45名**、全員に`wp`が加算されることを確認
- `teamResult: true`により`wins`/`podiums`が加算された選手数：**0**（設計どおり）

`scratchpad/w84_e2e_result.mjs` / `w84_e2e_result2.mjs`（`mlFinishTeamTT`を実際の
状態オブジェクトで直接実行）：

- 5位・グレード1 → `popGain: 0.3`、`riderStats`登録数50、`bonds`不変（無所属僚友0名）
- 1位・グレード1・僚友2名（絆40/60）→ `popGain: 1.5`、人気度24.7→26.2で
  マイルストーン25到達、`popBonus: +80万円`のログ生成、絆40/60→43/63（+3ずつ、
  podium加点込み）、`money`が賞金＋人気ボーナスぶん正しく加算

すべて設計どおりの値が出力されることを確認した。

### 検証方法についての補足

第83弾で「長時間の自動プレイは10分でタイムアウトした（イベント選択肢の語彙が
多様で汎用セレクタが進行ボタンを取りこぼす）」ことが判明済みのため、今回は
**実際にUIをクリックしてチームTTの月まで進める全機能テストは行っていない**
（`mlGenRaceCandidates`でチームTTが出る月は144ヶ月中20ヶ月とまばらで、
自動操作で到達させるコストが高い）。代わりに、`screens/mylife/race.jsx`が
`ml.resultInfo`から読む**全フィールド（`popGain`/`popBonus`/`worldRank`等）を、
実際のゲーム状態と実際のsim出力で`mlFinishTeamTT`を直接実行し、UIが消費する
契約と一致することを確認**した（上記e2eスクリプト）。追加したJSXは通常レース側の
既存の「人気度」Itemブロックと構造・propsが同一（コピー元は実機で見た目確認済み）
のため、レンダリング面のリスクは低いと判断している。⚠️**実際にブラウザで
チームTT結果画面を目視するテストは今回省略した**——次にマイライフの通しプレイを
行う機会（第83弾で未達となった「1キャリア通し」の再挑戦時等）に、この画面も
併せて目視確認することを推奨する。
