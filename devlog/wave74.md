...
**実装済み**。以下は実装前に固めた設計図（実装はこの通りに行った・差分は無し）。
...
実装ファイル：`data/course.js`（②③）／`domain/mylife/race.js`（①・第5引数focusSlots廃止し
focusを文字列/配列両対応に）／`state/{mylifeState,meta}.js`（①のフィールド）／
`domain/mylife/createChar.js`（①の初期化）／`hooks/useMyLifeGame.js`（①のトグルロジック）／
`screens/mylife/{hub,create}.jsx`（①のUI）／呼び出し側4箇所（`controllers/mylife/{month,career}.js`）。

## 検証1：単一宣言は変更前とバイト単位で同一

変更前の`race.js`をgit historyから復元し、`focus`が文字列（または長さ1の配列）のときの出力を
新実装と比較（`scratchpad/w74_compare_old_new.mjs`）。**5種の宣言×15年×12ヶ月＝900パターン、
不一致0件。** RNG消費順・候補の並び・全プロパティが完全一致。

## 検証2：2地形目が意図どおり並行して効く

| 宣言 | climb宣言のみ | climb+solo(2地形目) |
|---|---|---|
| climb取得月 | 99月 | 91月 |
| solo取得月 | 38月 | **86月** |

| 宣言 | hill宣言のみ | hill+climb(2地形目) |
|---|---|---|
| hill取得月 | 112月 | 110月 |
| climb取得月 | 49月 | **99月** |

**2地形目が無かった地形の取得月を2倍以上に押し上げ、主宣言側はわずかな減少（99→91等）に
留まる。** 死に商品だった旧`m_plan2`と違い、狙いどおり機能している。

## 検証3：グラベル改修後、favors不整合が解消

`terrainOfMix`のカウント軸で全9テンプレを再検証（`scratchpad/w74_final_table.mjs`）：
**グラベルレースが`hill`（favors:PUNと一致）になり、ズレは「スプリント宣言→flat集計」の
2件（クリテ／サーキット。ユーザー選択で今回は対象外）のみに減った。** 実地形分布は
flat3・hill2・climb2・solo2（ナイトクリテ撤去でflatの占有率50%→33%）。

## 検証4：ブラウザでの動作確認（Playwright・Chromium）

- キャラ作成の目標バッジ画面／マイライフホームの両方で、`m_plan2`未購入時は**現行と
  見た目が完全に同一**（「2つまで」ラベルは非表示、後から選んだ方に置き換わる）。
- `localStorage`へ`cpUnlocks:["m_plan2"]`を注入した状態では、**両画面で「2つまで」
  ラベルが表示され、2つ選択可能**。3つ目を選ぶと最も古い選択が外れてスライド
  （`丘陵中心`と`山岳中心`の2択→独走選択で丘陵が外れ`山岳・独走（TT）`に更新、を確認）。
- `mlSetRaceFocus`のトグル・PressRowの値表示（`丘陵・山岳中心`のように`・`で連結）・
  下部の説明文（`毎月の候補に丘陵・山岳のレースが必ず入る`）いずれも設計どおり。
- コンソールエラー無し。`npm run build`成功（デプロイ用`index.html`も更新済み）。

## 判定

①②③すべて実装完了。TODO #27-b・27-dはクローズ。旧セーブへの影響（`raceFocusSlots`は
`ML_SAVE_FIELDS`から外れ自動的に読み捨てられる）も設計どおりで、移行コードは不要だった。
