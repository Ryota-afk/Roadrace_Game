...
設計どおり§1〜§3を実装。座標は仮置きから以下の調整が必要だった：

- **玄関ホール**：hall-plant(8.4,-3.0)/hall-bench(10.5,-2.9)の仮置きは、mechanic/scout
  ルートが玄関ホールの中央を貫通する縦回廊(w=9.5)と、training/medical分岐の横帯(l=-2.7)
  の**両方**にクリアランス不足で引っかかった（`tools/verify_baseview.mjs`実行で4件NG）。
  玄関ホールは中央十字の回廊で4分割された狭い部屋のため、四隅の空きスペースへ寄せる
  必要があると判明。数値探索でhall-plant(8.3,-3.5)・hall-bench(10.7,-1.2)に確定＝ゼロ違反。
- 食堂・ロッカー・トロフィーの4座標は仮置きのまま検証器を通過（部屋の奥半分は歩行動線から
  離れているため、玄関ホールほど窮屈ではなかった）。

### 実機確認
- Node純関数検算：フレッシュ／解禁直後最小／全成長の3状態で`buildingLevels(g)`の
  hall/diner/locker/trophyが設計どおりの値になることを確認（trophyの二重計上なしも確認）。
- Playwright実機：3状態でpageerrorゼロ。**DOM上のワールド座標を直接読み取る検証**
  （第21弾で確立した手法）で、全成長状態の新什器8点すべてが設計どおりの座標に存在し、
  解禁直後最小状態では8点とも存在しないこと（＝既存の見た目のまま）を機械的に確認した。
  スクリーンショットの目視でも、ロッカー室のロッカー2列目＋着替えベンチ、トロフィー室の
  ケース2本目＋掲示板を確認できた。
- `node tools/verify_baseview.mjs`ゼロ違反、`npm run build`成功。
