...
## `FINISH_BADGE_K`の実測（2026-08・Opus）

測定方法：`FINISH_BADGE_K`は`export const`で外部から差し替えられないため、**測定の間だけ
ソースの定数を書き換え、終了後に元へ戻した**（`git diff`で復元を確認済み）。
主力power74/A・n=192・同一シード（3点固定）。バッジは`mount`/`allclimber`/`closer`の3枚。

| K | バッジ無し | 銅 | 銀 | 金 | 虹 | 銅→虹 |
|---|---|---|---|---|---|---|
| 1 | 22.4% | 26.0% | 24.5% | 26.0% | 27.1% | +1.0pt |
| **2** | 22.4% | **27.1%** | 24.5% | 26.6% | **28.1%** | +1.0pt |
| 3 | 22.4% | 27.1% | 24.5% | 26.6% | 28.1% | +1.0pt |
| 4 | 22.4% | 27.1% | 24.5% | 26.6% | 28.1% | +1.0pt |

3位以内（表彰台）率はより素直に反応した：**バッジ無し34.9% → 銅39.1% → 虹44.3%**。

### 結論1：第50弾の合流自体は成功している

**バッジ無し22.4% → 銅27.1%（+4.7pt）／表彰台34.9%→39.1%（+4.2pt）。**
第49弾では「バッジを3枚積んでも勝率が動かない」状態だったので、決着への合流が
バッジを実際に機能させたことが確認できた。

### 結論2：⚠️`K=2`で飽和する。3以上は完全に無意味

**K=2/3/4の数値が1桁まで完全に一致した。** 原因は構造的：`resolveFinishClusters`は
**同着集団の中でしか並べ替えない**ため、バッジのボーナスが「その集団で1位」に届いた
時点でそれ以上は何も変わらない。どの集団にいるか自体はtick側が決めており、決着の
重みをいくら上げても集団をまたぐ移動は起きない。

→ **`FINISH_BADGE_K = 2` を確定値とする。**（K=1より銅で+1.1pt・虹で+1.0pt良く、
K=3以上は完全に同じなので、効果が頭打ちになる最小の値がK=2）

### 結論3：⚠️段階差は依然として小さい（勝率+1.0pt／表彰台+5.2pt）

銅→虹の勝率差はどのKでも+1.0ptしかない（n=192では誤差内）。表彰台率では
銅39.1%→虹44.3%＝**+5.2pt**と反応しているので効果は実在するが、当初の目標
「銅→虹で+8〜10pt」には届いていない。

理由も同じ構造にある：バッジを**持っているだけ**で同着集団の上位に来るため、
段階による上乗せが「集団内1位」の判定を覆す場面が少ない。段階差をさらに広げるには
**tick側（どの集団に入るか）にバッジを効かせる**必要があり、これは決着への合流とは
別の変更になる。この弾のスコープ外として残す。
