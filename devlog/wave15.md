...
**状態：完了（2026-08）**。7番（配合面の深掘り）に着手し完結。あわせてCLAUDE.md §5に沿って
`state.js`（1,096行・当時最大）を分割した。DEVLOG.md §46が要約、本ファイルが全文。
...
**バランス較正（実装後に実測・結論：係数調整は不要）**：

実装した5種の伝説特能は、既存の`sireline`/`dynasty`/`hybrid`（配合限定）と同じ実装パターンを
踏襲し、**`r.flat`等の生ステータス（練習で伸びる永続値）自体は一切書き換えず**、`sim/race.js`の
`effAbilities`/`segmentAbility`（レース中に一時的に計算する実戦値）にのみ加算する。
`domain/mylife/growthCap.js`の`mlGrowthCap`（生ステータスの伸びしろの天井：
`min(140, 90 + 時間成分 + 実績成分 + player.talentCap)`）と`player.talentCap`は、
`mlBreedBonus`（既存の配合ボーナス）だけが決める値で、血脈レシピの成立ではこれらを
一切変更しない。実測（`scratchpad/w15_calib_supremacy.mjs`）で確認：
- 最深レシピ「覇道極まれり」(`sovereign`)を成立させた個体でも`player.talentCap`は配合由来の
  値のまま（レシピ成立による上乗せ0）。`mlGrowthCap`の値も無印個体と変わらない。
- 当初案が懸念していた「伝説特能の能力+6と衰え無効を重ねると上限140に容易に届く」という
  想定は、そもそも伝説特能がこの上限（生ステータス側）に触れない実装になっているため
  該当しない。
- 一方、実戦値（`effAbilities`）側には既存の全キャラ共通の上限（`Math.min(135, e[k])`）が
  既にあり、練習で生ステータスを100超まで育てた熟練個体＋高強化パーツ＋★3レースの
  組み合わせでは`sovereign`込みでもこの135上限に張り付く区間が生じる（実測：flat/climbが
  135着地）ことを確認した。これは伝説特能固有の問題ではなく、既存キャラ全員に共通する
  安全弁がそのまま働いているだけであり、育成途上（未到達）の個体では加算分がそのまま
  反映される。

**結論**：伝説特能の数値（+3〜+6、地形限定+4〜+6、消耗-8〜-10%、大舞台+5%、衰え抑制
0〜3割）はいずれの安全弁にも異常な形で衝突しないため、**係数の引き下げは行わない**。
