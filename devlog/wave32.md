...
## Phase C 全画面通し検証（2026-08・Opus）

弾の冒頭で取ったベースライン（本ファイル「実測」節）と同じ指標を、実際に描画されたDOMから
測り直した（`scratchpad/phase_c.mjs`）。
...
|---|---|
| `npm run build` | 通過 |
| `tools/verify_radar.mjs` | **全6項目OK**（distをport 8123で配信して実行する仕様） |
| `tools/verify_baseview.mjs` | OK（部屋の敷き詰め／収容／壁交差ゼロ／クリアランス／コース範囲） |
| 縦線（`borderLeft`/`borderRight`） | 実コードでの使用**ゼロ**（grep3件はいずれも「撤去した」旨のコメント） |
| `pageerror`（10画面通し） | **ゼロ** |

**絵文字：画面上は実質ゼロ**。`src/`のgrepでは190個ヒットするが、実際に描画されたDOMを
走査すると`★`（レースのグレード＝機能記号）と`☰`（ハンバーガーの開閉状態）の2種のみ。
残りはコメントか、**どこからも描画されていないデータ定義**だった：
`domain/season/achievements.js`・`breeding/breeding.js`・`data/economy.js`・`data/directives.js`・
`state/mylifeState.js`・`data/baseViewBuildings.js`・`domain/mylife/lineage.js`の`icon:`フィールドは
第13弾以降どの画面からも参照されていない（`.icon`を描画しているのは`panels.jsx`の2箇所だけで、
中身は下馬評の`◎○▲`）。**死にデータなので次の整理で削除候補**。

### 文字数・階層の前後比較

| 画面 | 文字数 前→後 | ボタン 前→後 | 最小級の文字が占める割合 前→後 |
|---|---|---|---|
| モード選択 | 131 → **52**（−60%） | 2 → 2 | 75% → **29%** |
| マイライフ キャラ作成 | 614 → 543（−12%） | 15 → 15 | 86% → 84% |
| マイライフ ホーム | 360 → **224**（−38%） | 22 → **13** | 62% → 68% |
| マイライフ 選手 | 331 → 293（−11%） | 6 → 6 | 70% → 67% |

（「最小級」＝ベースライン時は`caption`=12px、現在は`caption`=10px。どちらもその画面の
補足文字の階層を指す）

**達成できたこと**：文字の総量は全画面で減り、モード選択は**16px以上が71%**＝「名前と数字が
主役」の形になった。ホームはボタン22→13でタップ対象の絞り込みも効いている。実機の
スクリーンショットでも、ホームは総合力57（38px）→レース名（20px）→主ボタン（action塗り）と
視線の順路が明確についている。

**達成できていないこと（正直な結果）**：**「最小級の文字が画面を占める割合」は改善していない**。
ベースラインの診断は「画面の6〜9割を占める補足文字の平坦な敷き詰め」であり、処方は
「量を削り、残りに差をつけ直す」だった。量は削れたが、**比率はマイライフ側でほぼ動いていない**
（ホームは62%→68%とむしろ上昇）。captionが12px→10pxに縮んだだけで、同じ補足文字が
そのまま最小の階層に居続けている。

### 残っている密集画面（次弾の候補）

Phase Aは代表3画面（モード選択・ホーム・選手一覧）を作り直し、Phase Bは残りへ**変換ルールの
機械適用**を行った。その結果、**一度も作り直していない2画面が現在の最密**として残った：

| 画面 | 文字数 | 最小級 | 16px以上 | 状態 |
|---|---|---|---|---|
| マイライフ ショップ | **1045** | 77.2% | 20.8% | Phase Bで機械変換のみ。全画面中で最も文字が多い |
| マイライフ キャラ作成 | 543 | **84.2%** | **1.5%** | ベースライン時点の最悪画面（86%）から動いていない |

キャラ作成は**16px以上が1.5%＝画面に主役が存在しない**。脚質5件・経歴3件・難易度4件のすべてに
2行の小さな説明文が付き、実機で見ると小さい文字の壁になっている（`/tmp/phasec/ml_create.png`）。
ゲーム開始前の最初の意思決定画面がこれである点も含めて、次弾の最有力候補。
