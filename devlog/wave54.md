...
# Phase 2 実測（2026-08・Opus）→ ⚠️**撤退基準3つすべてに抵触。テンポは断念**

`scratchpad/w54_sweep_one.mjs`（撤退基準3つを同時に測る）でスイープした。n=96。
⚠️`TEMPO_BASE = 1.0`は導入前と完全に同一の挙動＝基準点。

| TEMPO_BASE | terrain/hold | 基準との差 | グループ数 | 基準比 | terrain: attack−hold |
|---|---|---|---|---|---|
| **1.0（基準）** | **12.93** | — | **6.86** | — | +12.25 |
| 0.96 | 16.00 | **+3.07** | 5.21 | **−24%** | +7.47 |
| 0.94 | 17.16 | **+4.23** | 5.03 | **−27%** | +8.71 |
| 0.90 | 19.20 | **+6.27** | 4.76 | **−31%** | +5.93 |
| 0.85 | 19.10 | **+6.17** | 4.62 | **−33%** | +5.84 |

**判定：3つの撤退基準すべてに、どの設定でも抵触した。**

| # | 撤退基準 | 結果 |
|---|---|---|
| 1 | `hold`が基準から1着を超えて悪化 | ❌ **最良の0.96でも+3.07** |
| 2 | グループ数が基準から2割超減る | ❌ **最良の0.96でも−24%** |
| 3 | `attack`−`hold`が+3を下回らない | ❌ **最小でも+5.84** |

⚠️**式を作り直してもフィールドの融合は止まらなかった**（6.86→5.21）。原因は
「逃げている選手だけ`paceMul=1`で対象外」という第54弾の中核そのもの——AI選手は
`role:"breakaway"`や中盤の自然発生アタックで`committedBreak`が立ち、**一度立つと
二度と下りない**ため、レースが進むほど「対象外の選手」が増える。彼らが本隊より速く走り、
結局は集団間の速度差が生まれてフィールドが混ざった。⚠️**membershipの決め方を変えても、
フィールドの一部だけを速く/遅くする限り同じ現象が起きる**というのが3回分の結論。

さらに`TEMPO_BASE=0.85`（逃げの優位は**17.6%**）でも`attack`は`hold`より**5.84着悪い**。
⚠️**現実的にあり得ない速度差を与えても逃げは成立しない**＝**速度は障壁ではない**。

## ⭐ 決定実験：本当の障壁は「エネルギー経済」だった

テンポを完全にOFF（`TEMPO_BASE=1.0`）にし、`energyDrain`内の`brk`
（`committedBreak`かつ`solo`/`attack`の選手に掛かる消耗割引。現行
`{ mtn:0.4, climb:0.45, hill:0.55, flat:0.75, sprint:0.85, tt:0.75 }`）**だけ**を
全地形一律の値に振って測った（`scratchpad/w54_energy.sh`）。

| brk | mid/hold | mid: attack−hold | terrain/hold | terrain: attack−hold | グループ数 | 逃げ切り勝ち |
|---|---|---|---|---|---|---|
| 現状(0.4〜0.85) | 10.95 | +4.59 | 12.93 | +12.25 | 6.86 | 3.1% |
| 0.45 | 11.04 | +4.31 | 12.48 | +11.79 | 6.88 | 3.1% |
| 0.25 | 10.84 | +2.26 | 12.66 | +4.87 | 6.60 | 11.5% |
| **0.22** | **10.84** | **+1.91** | **12.93** | **+1.43** | **6.74** | **10.4%** |
| 0.18 | 11.14 | +0.63 | 13.00 | **−3.74** | 6.71 | 13.5% |
| 0.15 | 11.02 | +0.44 | 12.98 | −5.17 | 6.73 | 15.6% |
| 0.12 | 11.16 | −0.25 | 13.76 | −6.80 | 6.73 | 24.0% |

⭐**これがテンポでは絶対に得られなかった性質を全て満たしている：**

- **`hold`の絶対水準が動かない**（brk=0.22でterrain 12.93＝基準と完全に同値、mid 10.84＝
  基準10.95とほぼ同じ）。⚠️**集団に一切触らないので、基準点を犠牲にしない。**
- **フィールドが融合しない**（グループ数6.74＝基準6.86とほぼ同じ）。⚠️**速度差を作らず
  消耗だけを変えるため、誰が千切れるかの選抜が壊れない。**
- **`attack`が僅差の選択肢になる**（terrain +1.43・mid +1.91＝平均では少し損だが上振れ
  のある賭けになる＝第52弾で定義した設計目標そのもの）。
- **逃げ切り勝ちが3.1%→10.4%**と現実的な水準になる。

⚠️**brk=0.18以下は行き過ぎ**（terrainで`attack`が−3.74＝holdより明確に良くなり、
今度は「常に仕掛けるのが正解」になる。§35で潰した失敗の裏返し）。

**→ 結論：テンポ（第53・54弾）は撤去し、`brk`の再較正で逃げを成立させる。**
⚠️ただし**この測定は山岳系レース（`mlGenRaceCandidates(...,"climb")`）でしか行っていない**。
現行`brk`の平坦0.75・スプリント0.85という高い値は今回ほとんど動かしていないため、
**平坦系コースでの確認測定が必須**。
